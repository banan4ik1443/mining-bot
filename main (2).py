...
def get_help_keyboard():
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🛒 Маркет",       callback_data="help_market"),
         InlineKeyboardButton(text="🔨 Аукцион",      callback_data="help_auction")],
        [InlineKeyboardButton(text="🎮 Игры",         callback_data="help_games"),
         InlineKeyboardButton(text="⚔️ Дуэли",       callback_data="help_duels")],
        [InlineKeyboardButton(text="🔥 Стрики",       callback_data="help_streaks"),
         InlineKeyboardButton(text="🏆 Турнир",       callback_data="help_tournament")],
        [InlineKeyboardButton(text="🎁 Подарки",      callback_data="help_gifts"),
...
async def callback_market(callback: CallbackQuery, state: FSMContext):
    """Главное меню Маркета — выбор раздела."""
    await callback.answer()
    await state.clear()
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🏪 Барахолка",  callback_data="market_flea")],
        [InlineKeyboardButton(text="🔨 Аукцион",    callback_data="auction_list")],
        [InlineKeyboardButton(text="« Назад",      callback_data="back_to_menu")],
    ])
    await callback.message.edit_text(
    ...
    while True:
        ...
        try:
            ...
            for (row_uid,) in active_users:
                ...
                try:
                    await bot.send_message(row_uid,
                        f"🌍 <b>Мировое событие!</b>\n\n{ev['title']}\n{ev['desc']}\n\n⏰ {ev['dur']} минут",
                        parse_mode="HTML")
                except Exception: pass
            if ev["effect"] == "instant_ton":
                db.conn.commit()
