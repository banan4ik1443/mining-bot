            ...
            if db.check_and_restock():
                ...
                if total > 0:
                    ...
                    for admin_id in config.ADMIN_IDS:
                        try:
                            await bot.send_message(admin_id, restock_text, parse_mode="HTML")
                        except:
                            pass
...
async def callback_help_market(callback: CallbackQuery):
    ...
    text = (
        ...
        "🏪 <b>Барахолка</b> — выставляй GPU и куллеры по фиксированной цене.\n"
...
async def callback_help_gifts(callback: CallbackQuery):
    ...
    text = (
        ...
        "/gift MID cooler — подарить куллер\n\n"
...
async def cmd_createpromo(message: Message):
    """/createpromo <код> [gems=0] [exp=0] [boxes=0] [uses=1]"""
    if not is_admin(message.from_user.id):
        await message.answer("❌ Нет прав!")
        return
    parts = message.text.split()
    if len(parts) < 2:
        await message.answer("Использование: /createpromo &lt;код&gt; [gems=0] [exp=0] [boxes=0] [uses=1]", parse_mode="HTML")
        return
    code = parts[1].upper()
    gems = 0; exp = 0; boxes = 0; max_uses = 1
    for p in parts[2:]:
        if "=" in p:
            k, v = p.split("=", 1)
            try:
    ...
    try:
        db.cursor.execute(
            "INSERT INTO resource_promos (code, gems, exp, boxes, max_uses, uses, created_by) VALUES (?,?,?,?,?,0,?)",
            (code, gems, exp, boxes, max_uses, message.from_user.id)
        )
        db.conn.commit()
        await message.answer(
            f"✅ Промокод <b>{code}</b> создан!\n"
            f"💎 Gems: {gems}  |  EXP: {exp}  |  📦 Боксы: {boxes}\n"
...
async def handle_promo_code_input(message: Message, state: FSMContext):
    ...
    promo = db.cursor.execute(
        "SELECT gems, exp, boxes, max_uses, uses FROM resource_promos WHERE code=?", (code,)
    ).fetchone()
    if not promo:
        await message.answer("❌ Промокод не найден.")
        return
    gems, exp, boxes, max_uses, uses = promo
    if uses >= max_uses:
    ...
    if exp:
        db.cursor.execute("UPDATE users SET experience=experience+? WHERE user_id=?", (exp, message.from_user.id))
    if boxes:
        db.cursor.execute("UPDATE users SET boxes=boxes+? WHERE user_id=?", (boxes, message.from_user.id))
    db.conn.commit()
    parts = []
    if gems: parts.append(f"💎 +{gems} gems")
    if exp: parts.append(f"⭐ +{exp} exp")
    if boxes: parts.append(f"📦 +{boxes} боксов")
    await message.answer(
...
async def callback_auc_sell_choose(callback: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="❄️ Куллер", callback_data="auc_inv_cooler")],
...
async def callback_auc_inv_cooler(callback: CallbackQuery):
    ...
    buttons.append([InlineKeyboardButton(text="« Назад", callback_data="auc_sell_choose")])
    await callback.message.edit_text(
        "❄️ Выбери куллер:" if buttons[:-1] else "❌ Нет свободных куллеров",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons), parse_mode="HTML"
    )
...
async def process_auc_start_price(message: Message, state: FSMContext):
    ...
    if itype == "gpu":
        ...
        row = c2.fetchone()
        if not row: await message.answer("❌ Куллер не найден"); return
...
async def cmd_gift(message: Message):
    ...
    if len(args) < 3:
        await message.answer(
            ...
            "<code>/gift MID cooler</code> — подарить куллер",
            parse_mode="HTML"); return
    ...
    if gift_type == "box":
        ...
        if not coolers:
            await message.answer("❌ Нет свободных куллеров"); return
        buttons = [[InlineKeyboardButton(
            text=f"❄️ {co[2]} | охл.{co[4] or 0}",
            callback_data=f"gift_cooler:{rec_id}:{co[0]}"
        )] for co in coolers[:10]]
        await message.answer(
            f"🎁 Выбери куллер для подарка {rec_name}:",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons),
            parse_mode="HTML")
    else:
...
async def callback_gift_cooler(callback: CallbackQuery):
    ...
    co = c2.fetchone()
    if not co:
        await callback.answer("❌ Куллер не найден", show_alert=True); return
    ...
    try:
        await bot.send_message(rec_id,
            f"🎁 <b>Тебе подарили куллер!</b>\n❄️ {co[0]}\nОт: <b>{me[3]}</b>",
            parse_mode="HTML")
    except Exception: pass
    await callback.message.edit_text(f"✅ Куллер «{co[0]}» подарен {rec[3]}!", parse_mode="HTML")
