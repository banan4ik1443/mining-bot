...
async def cmd_createpromo(message: Message):
    """/createpromo <код> [gems=0] [exp=0] [boxes=0] [stars=0] [parts=0] [uses=1]"""
    if not is_admin(message.from_user.id):
        await message.answer("❌ Нет прав!")
        return
    args = message.text.split()
    if len(args) < 2:
        await message.answer(
            "❌ Использование:\n<code>/createpromo &lt;код&gt; [gems=0] [exp=0] [boxes=0] [stars=0] [parts=0] [uses=1]</code>",
            parse_mode="HTML"
        )
        return
    code = args[1].upper()
    gems = 0; exp = 0; boxes = 0; stars = 0; parts = 0; max_uses = 1
    for p in args[2:]:
        if "=" in p:
            k, v = p.split("=", 1)
            try:
            ...
            elif k == "boxes": boxes = val
            elif k == "stars": stars = val
            elif k == "parts": parts = val
            elif k == "uses": max_uses = val
    db.cursor.execute(
