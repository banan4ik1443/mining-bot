...
def fmt(n) -> str:
    """Форматирует число с точками: 1000 → 1.000, 1000000 → 1.000.000"""
    try:
...
class GameStates(StatesGroup):
    ...
    flip_waiting_bet       = State()
    roulette_waiting_bet   = State()
    roulette_waiting_number = State()
...
class Database:
    def __init__(self):
        ...
        self.conn = sqlite3.connect('data/mining_bot.db', check_same_thread=False, timeout=30)
    ...
    def init_db(self):
        ...
        for _idx in [
            ...
            try:
                ...
                pass
        # Таблица баланса бота (комиссии)
        self.cursor.execute('''
            CREATE TABLE IF NOT EXISTS bot_balance (
                id INTEGER PRIMARY KEY DEFAULT 1,
                balance INTEGER DEFAULT 0
            )''')
        self.cursor.execute("INSERT OR IGNORE INTO bot_balance (id, balance) VALUES (1, 0)")
        self.conn.commit()
        # Миграция: стрики майнинга
        for col in [('mine_streak','INTEGER DEFAULT 0'),('mine_streak_date','TEXT DEFAULT NULL')]:
            try:
                self.cursor.execute(f'ALTER TABLE users ADD COLUMN {col[0]} {col[1]}')
                self.conn.commit()
        ...
        self.cursor.execute('''
            CREATE TABLE IF NOT EXISTS world_events (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                event_key TEXT, title TEXT, description TEXT,
                effect TEXT, magnitude REAL,
                started_at TIMESTAMP, ends_at TIMESTAMP,
                is_active INTEGER DEFAULT 1
            )''')
        ...
        self.cursor.execute('''
            CREATE TABLE IF NOT EXISTS auctions (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                seller_id INTEGER, item_type TEXT,
                item_id INTEGER, item_name TEXT,
                hash_rate REAL DEFAULT 0, wear INTEGER DEFAULT 100,
                start_price INTEGER, current_price INTEGER,
        ...
        self.cursor.execute('''
            CREATE TABLE IF NOT EXISTS duels (
                ...
                bet INTEGER, status TEXT DEFAULT 'pending',
        ...
        # Миграция: ресурсные промокоды (админ)
        self.cursor.execute(
            "CREATE TABLE IF NOT EXISTS resource_promos "
            "(id INTEGER PRIMARY KEY AUTOINCREMENT, code TEXT UNIQUE, "
            "gems INTEGER DEFAULT 0, exp INTEGER DEFAULT 0, boxes INTEGER DEFAULT 0, "
            "stars INTEGER DEFAULT 0, parts INTEGER DEFAULT 0, "
        ...
        for _col, _td in [
            ...
            try:
                self.cursor.execute(f'ALTER TABLE resource_promos ADD COLUMN {_col} {_td}')
            except Exception:
                pass
        self.cursor.execute(
            "CREATE TABLE IF NOT EXISTS resource_promo_uses "
            "(id INTEGER PRIMARY KEY AUTOINCREMENT, code TEXT, user_id INTEGER, used_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, "
            "UNIQUE(code, user_id))"
        )
    ...
    def _get_mine_streak(self, user_id: int) -> int:
        """Возвращает текущий стрик майнинга (дней подряд)."""
        c = self.conn.cursor()
        c.execute("SELECT mine_streak, mine_streak_date FROM users WHERE user_id=?", (user_id,))
        row = c.fetchone()
        ...
        try:
            ld = datetime.fromisoformat(last_date).date() if 'T' in str(last_date) else                  datetime.strptime(str(last_date), '%Y-%m-%d').date()
        except Exception:
            return 0
        diff = (today - ld).days
        if diff == 0:
            return int(streak or 0)   # сегодня уже майнили
        elif diff == 1:
            return int(streak or 0)   # вчера майнили — стрик продолжится
        else:
            return 0  # пропустили — стрик сброшен

    def _update_mine_streak(self, user_id: int):
        """Обновляет стрик после майнинга."""
        c = self.conn.cursor()
        c.execute("SELECT mine_streak, mine_streak_date FROM users WHERE user_id=?", (user_id,))
        row = c.fetchone()
        ...
        if not row or row[1] is None:
            self.cursor.execute(
                "UPDATE users SET mine_streak=1, mine_streak_date=? WHERE user_id=?",
                (today, user_id))
        else:
            streak, last_date = row
            try:
                ld = datetime.fromisoformat(str(last_date)).date() if 'T' in str(last_date) else                      datetime.strptime(str(last_date), '%Y-%m-%d').date()
            except Exception:
                ld = None
            t = datetime.now().date()
            if ld == t:
                pass  # уже обновляли сегодня
            elif ld and (t - ld).days == 1:
                self.cursor.execute(
                    "UPDATE users SET mine_streak=mine_streak+1, mine_streak_date=? WHERE user_id=?",
                    (today, user_id))
            else:
                self.cursor.execute(
                    "UPDATE users SET mine_streak=1, mine_streak_date=? WHERE user_id=?",
    ...
    def install_cooler(self, rig_id, cooler_id, slot_index):
        try:
            ...
            if not cooler:
                return False, "Кулер не найден или уже установлен"
            ...
            return True, f"✅ Кулер установлен! Хешрейт: {new_hash:.1f} MH/s"
        except Exception as e:
    ...
    def remove_cooler(self, rig_id, slot_index):
        try:
            ...
            return True, f"✅ Кулер извлечен! Хешрейт: {new_hash:.1f} MH/s"
        except Exception as e:
    ...
    def get_free_coolers(self, user_id):
        """Получает кулеры, не установленные в риги и не на барахолке"""
        self.cursor.execute('''
    ...
    def get_free_coolers_for_sale(self, user_id):
        """Только свободные кулеры (не в риге, не на барахолке)"""
        self.cursor.execute(
    ...
    def buy_cooler(self, user_id, cooler_key):
        try:
            ...
            self.conn.commit()
            return True, f"✅ Куплен кулер: {cooler[1]}"
        except Exception as e:
    ...
    def create_check(self, creator_id, amount, activations, target_username=None, comment=None, check_type='check'):
        ...
        try:
            ...
            if check_type == 'check' and user[4] < total_cost:
                return False, f"❌ Недостаточно тон! Нужно: {fmt(total_cost)}, у тебя: {fmt(user[4])}"
        ...
        try:
            ...
            if check_type == 'invoice':
                ...
                if not payer or payer[4] < amount:
                    return False, f"❌ Недостаточно тон! Нужно: {fmt(amount)}"
    ...
    def transfer_tons(self, from_user_id, to_user_id, amount, comment=None):
        ...
        try:
            ...
            if amount <= 0:
                return False, "❌ Сумма должна быть больше нуля"
            if sender[4] < amount:
                return False, f"❌ Недостаточно тон! У тебя: {fmt(sender[4])} тон"
    ...
    def mine(self, user_id):
        ...
        # Мировое событие
        _ev = get_active_event()
        if _ev and _ev["effect"] == "ton_mult":
            raw_ton = int(raw_ton * _ev["magnitude"])
        # Стрик-бонус: +1% за каждый день подряд (макс ×2)
        streak = self._get_mine_streak(user_id)
        streak_mult = min(2.0, 1.0 + streak * 0.01)
        base_ton = max(1, int(raw_ton * streak_mult))
        exp_gain = config.EXP_FROM_MINING_BASE + int(total_hash * config.EXP_FROM_HASHRATE)
        self._update_mine_streak(user_id)
        ...
        if rigs:
            for rig in rigs:
                ...
                if gpus:
                    for gpu in gpus:
                        try:
                            is_crafted = gpu[9] if len(gpu) > 9 and gpu[9] is not None else 0
                            if is_crafted:
                                continue
    ...
    def open_box(self, box_id):
        ...
        lines = []
        lines.append(f"💰 {fmt(ton_reward)} тон")
    ...
    def get_top(self, category):
        ...
        try:
            if category == "gems":
                ...
                self.cursor.execute('''
                    SELECT user_id, username, first_name, mine_streak 
                    FROM users 
                    WHERE mine_streak > 0
                    ORDER BY mine_streak DESC LIMIT 10
                ''')
            elif category == "duels":
                self.cursor.execute('''
                    SELECT u.user_id, u.username, u.first_name,
                           COUNT(d.id) as wins
                    FROM users u
                    JOIN duels d ON d.winner_id = u.user_id
    ...
    def add_gems(self, user_id, amount, admin_id=None):
        self.cursor.execute('UPDATE users SET gems = gems + ? WHERE user_id = ?', (amount, user_id))
        if admin_id:
            self.log_admin_action(admin_id, 'add_gems', user_id, f'+{fmt(amount)} тон')
        self.conn.commit()
    ...
    def level_up(self, user_id):
        ...
        if current_ton < ton_cost:
            return False, f"Недостаточно тон! Нужно {fmt(ton_cost)}, у тебя {fmt(current_ton)}"
    ...
    def remove_gems(self, user_id, amount, admin_id):
        self.cursor.execute('UPDATE users SET gems = gems - ? WHERE user_id = ?', (amount, user_id))
        self.log_admin_action(admin_id, 'remove_gems', user_id, f'-{fmt(amount)} тон')
        self.conn.commit()
    ...
    def market_list_item(self, seller_id, item_type, item_id, price, comp_name=None, comp_amount=None):
        ...
        conn2 = _sqlite3.connect('data/mining_bot.db', timeout=30)
        ...
        try:
            ...
            if item_type == 'gpu':
                ...
                row = c.fetchone()
                if not row:
                    return False, "❌ Кулер не найден или установлен в риг"
    ...
    def market_buy_item(self, buyer_id, listing_id):
        ...
        try:
            ...
            # Снимаем деньги у покупателя, 5% комиссия боту
            commission = max(1, int(price * 0.05))
            seller_gets = price - commission
            c.execute('UPDATE users SET gems = gems - ? WHERE user_id = ?', (price, buyer_id))
            c.execute('UPDATE users SET gems = gems + ? WHERE user_id = ?', (seller_gets, seller_id))
            c.execute('UPDATE bot_balance SET balance = balance + ? WHERE id = 1', (commission,))
    ...
    def market_buy_component(self, buyer_id, listing_id):
        ...
        try:
            ...
            if not buyer or buyer[4] < total_price:
                return False, f"❌ Нужно {fmt(total_price)} тон"
...
def _advance_streak_quest(user_id: int, streak_value: int) -> list:
    """
    Специальная версия для стрик-заданий.
    Устанавливает прогресс = streak_value (не прибавляет),
    чтобы нельзя было накрутить майнингом за одну сессию.
    """
    data = get_user_quests(user_id)
    completed = []
    changed = False
    for group in ("daily", "weekly"):
        for q in data.get(group, []):
            if q["type"] == "streak" and not q["done"]:
                q["progress"] = min(streak_value, q["target"])
                if q["progress"] >= q["target"] and not q.get("claimed"):
                    q["done"] = True
                    q["claimed"] = True
                    db.add_gems(user_id, q["reward_ton"])
                    db.cursor.execute(
...
async def _show_quests(user_id: int, target, group: str = "daily"):
    ...
    nav.append(InlineKeyboardButton(text="« Назад", callback_data="profile"))
    buttons.append(nav)
...
async def cmd_givestars(message: Message):
    """/givestars <MID|@user> <кол-во> — выдать звёзды"""
    if not is_admin(message.from_user.id):
        return
    args = message.text.split()
    if len(args) < 3 or not args[2].lstrip("-").isdigit():
        await message.answer("❌ Использование: <code>/givestars &lt;MID|@user&gt; &lt;кол-во&gt;</code>", parse_mode="HTML")
        return
    ...
    db.conn.commit()
    db.log_admin_action(message.from_user.id, "givestars", uid, f"Выдано {amount} ★")
    await message.answer(
        f"✅ <b>Звёзды {'выданы' if amount >= 0 else 'сняты'}!</b>\n"
        f"👤 {escape_html(target_name)} (MID: {user[1]})\n"
        f"⭐ Изменение: {'+' if amount>=0 else ''}{amount} ★",
        parse_mode="HTML"
    )
    try:
        await bot.send_message(uid, f"⭐ Вам {'выданы' if amount>=0 else 'сняты'}: {'+' if amount>=0 else ''}{amount} ★ от администратора!")
    except Exception:
        pass
...
def get_main_keyboard():
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            InlineKeyboardButton(text="➕ Добавить бота в чат", url=f"https://t.me/{config.BOT_USERNAME}?startgroup=true"),
        ],
    ])
    return keyboard
...
def get_help_keyboard():
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🛒 Магазин",        callback_data="help_shop"),
         InlineKeyboardButton(text="🎮 Игры/Дуэли",  callback_data="help_games")],
        [InlineKeyboardButton(text="👥 Рефералы",     callback_data="help_referrals"),
         InlineKeyboardButton(text="🏆 Турниры/Топ",  callback_data="help_tournament")],
        [InlineKeyboardButton(text="⭐ Донат",        callback_data="help_donate"),
         InlineKeyboardButton(text="📦 Прочее",       callback_data="help_boxes")],
...
def get_back_to_help_keyboard():
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="« Назад в помощь", callback_data="back_to_help")]
    ])
    return keyboard
...
def get_donate_keyboard():
    """Главное меню доната — только привилегии."""
    rows = [
        [
            InlineKeyboardButton(text="👑 Премиум — 50 ⭐",   callback_data="donate_buy:premium"),
            InlineKeyboardButton(text="💎 VIP — 125 ⭐",      callback_data="donate_buy:vip"),
        ],
        [
            InlineKeyboardButton(text="🌟 Легенда — 500 ⭐",  callback_data="donate_buy:legend"),
        ],
        [
...
async def _show_priv_info(target, priv_key: str):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="« Донат магазин", callback_data="donate_menu")],
    ])
    if isinstance(target, Message):
        await target.answer(text, reply_markup=kb, parse_mode="HTML")
    else:
...
def _donate_main_text(user, current_privilege, until) -> str:
    ...
    return (
        f"🪙 <b>Донат магазин</b>\n\n"
        f"<blockquote>"
        f"👑 Премиум » 50 ⭐ — бонусы к майнингу на 30 дней\n"
        f"💎 VIP » 125 ⭐ — расширенные бонусы на 30 дней\n"
        f"🌟 Легенда » 500 ⭐ — максимальные бонусы на 30 дней"
        f"</blockquote>\n\n"
        f"Привилегия: {priv_line}\n"
        f"Баланс: {fmt(user[4])} тон | {fmt(stars)} ⭐\n\n"
        f"Выберите привилегию:"
    )
...
async def _show_priv_or_bundle(target, key: str):
    ...
    if key in DONATE_PRIV_KEYS:
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="« Назад", callback_data="donate_buy_menu")],
        ])
        if isinstance(target, Message):
            await target.answer(text, reply_markup=kb, parse_mode="HTML")
        else:
...
async def callback_donate_buy(callback: CallbackQuery):
    """Покупка привилегии или набора за звёзды с баланса"""
    await callback.answer()
    key = callback.data.split(":")[1]
    if key in DONATE_PRIV_KEYS:
        await _buy_with_stars(callback, "priv", key)
    elif key in DONATE_BUNDLE_KEYS:
        await _buy_with_stars(callback, "bundle", key)


async def _buy_with_stars(callback: CallbackQuery, item_type: str, key: str):
    """Покупка за звёзды с баланса. Если не хватает — ссылка на оплату TG Stars."""
    uid = callback.from_user.id
    if item_type == "priv":
        p = config.PRIVILEGES.get(key)
        if not p: await callback.answer("❌ Не найдено", show_alert=True); return
        price = p["price"]
        name  = f"{p['icon']} {p['name']}"
    else:
        bun = BUNDLES.get(key)
        if not bun: await callback.answer("❌ Не найдено", show_alert=True); return
        price = bun["stars"]
        name  = f"📦 {bun['name']}"

    stars = db.get_stars(uid)

    if stars >= price:
        # Достаточно звёзд — покупаем сразу
        db.cursor.execute("UPDATE users SET stars_balance=stars_balance-? WHERE user_id=?", (price, uid))
        db.conn.commit()
        if item_type == "priv":
            ok, msg = db.apply_privilege(uid, key, 30)
        else:
            ok, msg = True, f"✅ Набор «{bun['name']}» активирован!"
            # Выдаём предметы набора
            bun_items = BUNDLES[key].get("items", [])
            for item_desc in bun_items:
                if "тон" in item_desc.lower():
                    import re as _re
                    m = _re.search(r"(\d+)", item_desc)
                    if m: db.add_gems(uid, int(m.group(1)))
        if ok:
            text = f"✅ <b>Куплено!</b>\n\n{name}\n💸 -{price} ⭐ с баланса"
        else:
            db.cursor.execute("UPDATE users SET stars_balance=stars_balance+? WHERE user_id=?", (price, uid))
        ...
        # Не хватает — показываем сколько нужно докупить и ссылку
        need = price - stars
        tg_stars_url = f"https://t.me/stars?amount={need}"
        text = (
            f"⭐ <b>Недостаточно звёзд</b>\n\n"
            f"Товар: {name}\n"
            f"Цена: {price} ⭐\n"
            f"На балансе: {fmt(stars)} ⭐\n"
            f"Не хватает: {need} ⭐\n\n"
            f"Пополни баланс через Telegram Stars:"
        )
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=f"⭐ Купить {need} звёзд в Telegram", url=tg_stars_url)],
            [InlineKeyboardButton(text="👛 Мой кошелёк", callback_data="donate_wallet")],
            [InlineKeyboardButton(text="« Назад", callback_data="donate_menu")],
        ])
        try:
            await callback.message.edit_text(text, reply_markup=kb, parse_mode="HTML")
...
@dp.callback_query(F.data.in_({"buy_premium","buy_vip","buy_legend"}))
async def callback_buy_priv_direct(callback: CallbackQuery):
    await callback.answer()
...
async def _do_stars_exchange(target, amount: int):
    ...
    if stars_bal >= amount:
        ...
        text = (
            ...
            f"⭐ {amount} звёзд → 💰 {fmt(ton_gain)} тон\n\n"
...
def get_shop_menu_keyboard():
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [
            ...
            InlineKeyboardButton(text="💨 Кулеры", callback_data="shop_coolers")
        ],
        [
...
def get_top_keyboard():
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="💰 Топ по тонам", callback_data="top_gems"),
...
def get_mycards_keyboard(user_id, items, page=0):
    ...
    buttons.append(nav_buttons)
    buttons.append([InlineKeyboardButton(text="« Назад в профиль", callback_data="profile")])
...
def get_boxes_keyboard(user_id, boxes, page=0, is_own=True):
    ...
    if is_own:
        ...
        buttons.append([InlineKeyboardButton(text="« Назад к профилю", callback_data=f"view_profile_{user_id}")])
...
def get_rig_detail_keyboard(rig_id, gpu_slots, cooler_slots, gpu_installed, coolers_installed):
    ...
    # Слоты для GPU (по 4 в строку, чтобы влезали все при 9+ слотах)
    gpu_row = []
    for i in range(gpu_slots):
        ...
        gpu_row.append(InlineKeyboardButton(text=icon, callback_data=f"rig_gpu_slot_{rig_id}_{i}"))
        if len(gpu_row) == 4:
            buttons.append(gpu_row)
            gpu_row = []
    if gpu_row:
        buttons.append(gpu_row)
    
    # Слоты для кулеров (по 4 в строку)
    if cooler_slots > 0:
        cooler_row = []
        for i in range(cooler_slots):
            ...
            cooler_row.append(InlineKeyboardButton(text=icon, callback_data=f"rig_cooler_slot_{rig_id}_{i}"))
            if len(cooler_row) == 4:
                buttons.append(cooler_row)
                cooler_row = []
        if cooler_row:
            buttons.append(cooler_row)
    
    buttons.append([InlineKeyboardButton(text="« Назад к списку", callback_data="my_rigs")])
...
def get_rig_gpu_slot_keyboard(rig_id, slot_index, is_empty, gpu_id=None, is_broken=False):
    ...
    buttons.append([InlineKeyboardButton(text="« Назад к ригу", callback_data=f"rig_detail_{rig_id}")])
    
    return InlineKeyboardMarkup(inline_keyboard=buttons)

def get_rig_cooler_slot_keyboard(rig_id, slot_index, is_empty, cooler_id=None, is_broken=False):
    """Клавиатура для слота кулера в риге"""
    buttons = []
    
    if is_empty:
        buttons.append([InlineKeyboardButton(text="➕ Установить кулер", callback_data=f"install_cooler_{rig_id}_{slot_index}")])
    else:
        if is_broken:
            buttons.append([InlineKeyboardButton(text="🔧 Починить кулер", callback_data=f"repair_cooler_{cooler_id}")])
        buttons.append([InlineKeyboardButton(text="📤 Вытащить кулер", callback_data=f"remove_cooler_{rig_id}_{slot_index}")])
...
def get_asic_detail_keyboard(asic_id, is_broken=False):
    ...
    buttons = []
    buttons.append([InlineKeyboardButton(text="« Назад к списку", callback_data="my_cards")])
    return InlineKeyboardMarkup(inline_keyboard=buttons)
    ...
    while True:
        try:
            if db.check_and_restock():
                ...
                if total > 0:
                    restock_text = f"📦 <b>Пополнение магазина</b>\n\n✅ Добавлено: {total} товаров\n\n" + "\n".join(logs[:15])
                    if len(logs) > 15:
                        restock_text += f"\n... и ещё {len(logs) - 15} позиций"
                    try:
                        await bot.send_message(config.CHANNEL_USERNAME, restock_text, parse_mode="HTML")
                        print(f"✅ Уведомление о пополнении отправлено в {config.CHANNEL_USERNAME}")
                    except Exception as ch_e:
                        print(f"⚠️ Не удалось отправить в канал {config.CHANNEL_USERNAME}: {ch_e}")
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    # Deeplink: /start duel_USERID_BET — принять дуэль от инлайна
    if len(args) > 1 and args[1].startswith("duel_"):
        db.add_user(message.from_user.id, message.from_user.username, message.from_user.first_name)
        try:
            _, chal_id_str, bet_str = args[1].split("_")
            chal_id = int(chal_id_str)
            bet = int(bet_str)
        except Exception:
            await message.answer("❌ Неверная ссылка дуэли")
            return
        if chal_id == message.from_user.id:
            await message.answer("❌ Нельзя принять собственный вызов")
            return
        chal = db.get_user(chal_id)
        opp  = db.get_user(message.from_user.id)
        if not chal:
            await message.answer("❌ Игрок не найден")
            return
        if int(chal[4]) < bet:
            await message.answer(f"❌ У вызывающего не хватает тон (нужно {fmt(bet)})")
            return
        if int(opp[4]) < bet:
            await message.answer(f"❌ У тебя не хватает тон (нужно {fmt(bet)})")
            return
        # Проверяем балансы
        if int(chal[4]) < bet:
            await message.answer(f"❌ У вызывающего не хватает тон (нужно {fmt(bet)}, есть {fmt(chal[4])})")
            return
        if int(opp[4]) < bet:
            await message.answer(f"❌ У тебя не хватает тон (нужно {fmt(bet)}, есть {fmt(opp[4])})")
            return
        # Создаём дуэль
        db.cursor.execute(
            "INSERT INTO duels (challenger_id,opponent_id,bet,status) VALUES (?,?,?,'finished')",
            (chal_id, message.from_user.id, bet)
        )
        duel_id = db.cursor.lastrowid
        db.conn.commit()
        # Списываем ставки
        db.cursor.execute("UPDATE users SET gems=gems-? WHERE user_id=?", (bet, chal_id))
        db.cursor.execute("UPDATE users SET gems=gems-? WHERE user_id=?", (bet, message.from_user.id))
        db.conn.commit()
        # Анимация кубиков
        await message.answer("⚔️ Дуэль начинается! Бросаем кубики...")
        d1 = await bot.send_dice(message.chat.id, emoji="🎲")
        await asyncio.sleep(0.2)
        d2 = await bot.send_dice(message.chat.id, emoji="🎲")
        await asyncio.sleep(3)
        # d1 = бросок создателя дуэли, d2 = бросок принявшего
        chal_roll = d1.dice.value
        opp_roll  = d2.dice.value
        while chal_roll == opp_roll:
            chal_roll = random.randint(1, 6)
            opp_roll  = random.randint(1, 6)
        winner_id = chal_id if chal_roll > opp_roll else message.from_user.id
        loser_id  = message.from_user.id if winner_id == chal_id else chal_id
        commission = max(1, int(bet * 2 * 0.05))
        prize = bet * 2 - commission
        db.cursor.execute("UPDATE users SET gems=gems+? WHERE user_id=?", (prize, winner_id))
        db.cursor.execute("UPDATE bot_balance SET balance=balance+? WHERE id=1", (commission,))
        db.cursor.execute("UPDATE duels SET winner_id=? WHERE id=?", (winner_id, duel_id))
        db.conn.commit()
        w = db.get_user(winner_id)
        l = db.get_user(loser_id)
        result_text = (
            f"⚔️ <b>Дуэль завершена!</b>\n\n"
            f"<blockquote>"
            f"🏆 {escape_html(w[3])}: 🎲 {chal_roll if winner_id == chal_id else opp_roll}\n"
            f"💀 {escape_html(l[3])}: 🎲 {opp_roll if winner_id == chal_id else chal_roll}"
            f"</blockquote>\n\n"
            f"Победитель: <b>{escape_html(w[3])}</b>\n"
            f"💰 Приз: {fmt(prize)} тон"
        )
        await message.answer(result_text, reply_markup=get_main_keyboard(), parse_mode="HTML")
        # Уведомляем вызывающего + кубики ему
        try:
            await bot.send_dice(chal_id, emoji="🎲")
            await bot.send_dice(chal_id, emoji="🎲")
            await asyncio.sleep(0.5)
            await bot.send_message(chal_id, result_text, parse_mode="HTML")
        except Exception: pass
        return
    ...
    if len(args) > 1 and args[1].startswith("check_"):
        ...
        if success:
            ...
            acts_left  = result["activations_left"]
            creator_name = creator[3] if creator else "Неизвестно"
            activator_name = escape_html(message.from_user.first_name or "Игрок")
            if check_type == "check":
                text = (
                    f"✅ <b>Чек #{code} активирован!</b>\n\n"
                    f"<blockquote>"
                    f"💰 Получено: <b>+{fmt(amount)} тон</b>\n"
                    f"👤 От: {escape_html(creator_name)}"
                    f"</blockquote>"
                )
                notify = (
                    f"🔔 <b>Чек #{code} активирован!</b>\n"
                    f"👤 {activator_name} получил {fmt(amount)} тон\n"
                    f"🔁 Осталось активаций: {acts_left}"
                )
                try:
                    await bot.send_message(creator[0], notify, parse_mode="HTML")
                except Exception: pass
            else:
                text = (
                    f"✅ <b>Счёт #{code} оплачен!</b>\n\n"
                    f"<blockquote>"
                    f"💸 Оплачено: <b>{fmt(amount)} тон</b>\n"
                    f"👤 Получатель: {escape_html(creator_name)}"
                    f"</blockquote>"
                )
                notify = (
                    f"💰 <b>Счёт #{code} оплачен!</b>\n"
                    f"👤 {activator_name} оплатил {fmt(amount)} тон\n"
                    f"🔁 Осталось: {acts_left}"
                )
                try:
                    await bot.send_message(creator[0], notify, parse_mode="HTML")
                except Exception: pass
    ...
    _existing = db.get_user(message.from_user.id)

    if _existing and referrer_id:
        # Уже в игре — просто показываем главное меню
        await message.answer(
            "ℹ️ Ты уже зарегистрирован в игре.\n"
            "Реферальная ссылка действует только при первом входе.",
...
async def cmd_mine(message: Message):
    ...
    if success:
        ...
        if success:
            import re as _re
            _m = _re.search(r"Тон ›› (\d+)", result)
            if _m:
                _earned = int(_m.group(1))
                _add_tournament_score(message.from_user.id, _earned)
                _qc += advance_quest(message.from_user.id, "earn", _earned)
                # Стрик-квест: устанавливаем прогресс = текущий стрик
                _streak_val = db._get_mine_streak(message.from_user.id)
                _streak_completed = _advance_streak_quest(message.from_user.id, _streak_val)
                _qc += _streak_completed
        for _qq in _qc:
            await message.answer(
...
@dp.callback_query(F.data == "help_market")
async def callback_help_market(callback: CallbackQuery):
    await callback.answer()
    text = (
        "🛒 <b>Маркет</b>\n\n"
        "<blockquote>"
        "Маркет делится на два раздела:\n\n"
        "🏪 <b>Барахолка</b> — выставляй GPU и кулеры по фиксированной цене.\n"
        "Комиссия: 5% от суммы продажи.\n"
        "Срок лота: 3 дня.\n\n"
        "🔨 <b>Аукцион</b> — торги за лучшую цену.\n"
        "Можно ставить начальную цену и цену быстрой покупки.\n"
        "Каждый новый лидер перебивает предыдущую ставку.\n"
        "При перебитии ставки деньги возвращаются автоматически.\n"
        "Длительность аукциона: 24 часа."
        "</blockquote>\n\n"
        "Команда: /market"
    )
    await callback.message.edit_text(text, reply_markup=get_back_to_help_keyboard(), parse_mode="HTML")


@dp.callback_query(F.data == "help_auction")
async def callback_help_auction(callback: CallbackQuery):
    await callback.answer()
    text = (
        "🔨 <b>Аукцион</b>\n\n"
        "<blockquote>"
        "Как участвовать:\n"
        "• Перейди в Маркет → Аукцион\n"
        "• Найди интересный лот и сделай ставку\n"
        "• Минимальная ставка: текущая цена +5%\n"
        "• Если тебя перебили — деньги вернутся автоматически\n\n"
        "Как выставить лот:\n"
        "• Нажми ➕ Выставить лот\n"
        "• Выбери GPU или кулер\n"
        "• Укажи стартовую цену\n"
        "• Опционально — цену мгновенной покупки\n\n"
        "Победитель определяется по истечении 24 часов."
        "</blockquote>"
    )
    await callback.message.edit_text(text, reply_markup=get_back_to_help_keyboard(), parse_mode="HTML")
...
async def callback_help_games(callback: CallbackQuery):
    await callback.answer()
    text = (
        "🎮 <b>Мини-игры</b>\n\n"
        "<blockquote>"
        "🎰 <b>Слоты</b> — крути барабан:\n"
        "  💎×10 | 7️⃣×7 | ⭐×5 | 🔔×3 | 🍊×2.5 | 🍋×2 | 🍒×1.5\n\n"
        "🎲 <b>Кубы</b> — подбрось монету:\n"
        "  Выигрыш ×1.9 при угадывании\n\n"
        "🎡 <b>Рулетка</b>:\n"
        "  🔴/⚫ Красное/Чёрное — ×1.9\n"
        "  🟢 Зеро — ×14\n"
        "  🎯 Число (0–36) — ×35\n\n"
        "Мин. ставка: 10 тон | Макс: 10 000 тон\n"
        "5% с проигрышей — комиссия."
        "</blockquote>\n\n"
        "Команды: /game /slots /flip /roulette"
    )
    await callback.message.edit_text(text, reply_markup=get_back_to_help_keyboard(), parse_mode="HTML")


@dp.callback_query(F.data == "help_duels")
async def callback_help_duels(callback: CallbackQuery):
    await callback.answer()
    text = (
        "⚔️ <b>Дуэли</b>\n\n"
        "<blockquote>"
        ...
        "/duel MID СТАВКА\n\n"
        "Как это работает:\n"
        "• Оба игрока бросают по одному кубику 🎲\n"
        "• У кого выпало больше — тот победитель\n"
        "• При ничьей кубики перебрасываются\n"
        "• Победитель получает обе ставки минус 5% комиссии\n\n"
        "MID игрока можно найти в его профиле."
        "</blockquote>\n\n"
        "Команда: /duel MID СТАВКА"
    )
    await callback.message.edit_text(text, reply_markup=get_back_to_help_keyboard(), parse_mode="HTML")


@dp.callback_query(F.data == "help_streaks")
async def callback_help_streaks(callback: CallbackQuery):
    await callback.answer()
    text = (
        "🔥 <b>Стрики майнинга</b>\n\n"
        "<blockquote>"
        "Майни каждый день подряд и получай бонус:\n\n"
        "• Каждый день стрика = +1% к доходу\n"
        "• Максимальный бонус: ×2.0 (100 дней)\n"
        "• Если пропустил день — стрик сбрасывается\n\n"
        "Пример:\n"
        "  7 дней → ×1.07 к тонам\n"
        "  30 дней → ×1.30 к тонам\n"
        "  100+ дней → ×2.00 к тонам\n\n"
        "Стрик отображается в результате майнинга."
        "</blockquote>"
    )
    await callback.message.edit_text(text, reply_markup=get_back_to_help_keyboard(), parse_mode="HTML")


@dp.callback_query(F.data == "help_tournament")
async def callback_help_tournament(callback: CallbackQuery):
    await callback.answer()
    text = (
        "🏆 <b>Еженедельный турнир</b>\n\n"
        "<blockquote>"
        "Каждую неделю проходит турнир по количеству намайненных тон.\n\n"
        "Призовой фонд: 10 000 тон\n"
        "🥇 1 место — 50% (5 000 тон)\n"
        "🥈 2 место — 30% (3 000 тон)\n"
        "🥉 3 место — 20% (2 000 тон)\n\n"
        "Победители получают приз автоматически.\n"
        "Турнир сбрасывается каждый понедельник."
        "</blockquote>\n\n"
        "Команда: /tournament"
    )
    await callback.message.edit_text(text, reply_markup=get_back_to_help_keyboard(), parse_mode="HTML")
...
@dp.callback_query(F.data == "help_gifts")
async def callback_help_gifts(callback: CallbackQuery):
    await callback.answer()
    text = (
        "🎁 <b>Подарки</b>\n\n"
        "<blockquote>"
        "Отправь подарок другому игроку по его MID:\n\n"
        "/gift MID box — подарить бокс\n"
        "/gift MID gpu — подарить GPU (выбор из склада)\n"
        "/gift MID cooler — подарить кулер\n\n"
        "Получатель получит уведомление с твоим именем.\n"
        "MID игрока можно найти в его профиле."
        "</blockquote>"
    )
    await callback.message.edit_text(text, reply_markup=get_back_to_help_keyboard(), parse_mode="HTML")
...
@dp.callback_query(F.data == "help_donate")
async def callback_help_donate(callback: CallbackQuery):
    await callback.answer()
    text = (
        "⭐ <b>Донат и привилегии</b>\n\n"
        "<blockquote>"
        "Покупка за звёзды Telegram:\n\n"
        "👑 <b>Премиум</b> — 50 ⭐ / 30 дней\n"
        "  ×1.5 доход, ×1.2 опыт, -10% ремонт\n\n"
        "💎 <b>VIP</b> — 125 ⭐ / 30 дней\n"
        "  ×2.0 доход, ×1.5 опыт, -25% ремонт\n\n"
        "🌟 <b>Легенда</b> — 500 ⭐ / 30 дней\n"
        "  ×3.0 доход, ×2.0 опыт, -50% ремонт\n\n"
        "Если звёзд не хватает — бот покажет ссылку на пополнение в Telegram.\n"
        "Звёзды хранятся на балансе в кошельке."
        "</blockquote>\n\n"
        "Команда: /donate"
    )
    await callback.message.edit_text(text, reply_markup=get_back_to_help_keyboard(), parse_mode="HTML")
...
async def cmd_referrals(message: Message):
    ...
    text = (
        ...
        f"├ 👤 Рефералов: {fmt(referrals_count)}\n"
...
async def cmd_inventory(message: Message):
    ...
    text += f"💨 Кулеры: всего {len(coolers) if coolers else 0} шт. "
    ...
    try:
        ...
        try:
            await message.bot.send_message(
                target_user[0],
                f"💰 Вам начислено {fmt(amount)} тон администратором!"
            )
        except:
            pass
...
async def cmd_take(message: Message):
    ...
    try:
        ...
        if target_user[4] < amount:
            await message.answer(f"❌ У пользователя недостаточно тон! Баланс: {fmt(target_user[4])}")
            return
        
        db.remove_gems(target_user[0], amount, message.from_user.id)
        
        await message.answer(
            f"✅ Забрано {fmt(amount)} тон\n"
            f"Пользователь: {target_user[3]} (MID: {target_user[1]})\n"
            f"💰 Новый баланс: {target_user[4] - amount:.0f} тон"
        )
        ...
        try:
            await message.bot.send_message(
                target_user[0],
                f"💸 У вас списано {fmt(amount)} тон администратором!"
            )
        except:
            pass
...
async def callback_mkt_filter_menu(callback: CallbackQuery):
    ...
    filter_labels = {"all": "Все", "gpu": "GPU", "cooler": "Кулеры", "component": "Компоненты"}
    ...
    # Раздел: Барахолка / Аукцион
    buttons.append([
        InlineKeyboardButton(text="🏪 Барахолка", callback_data=f"market_flea"),
        InlineKeyboardButton(text="🔨 Аукцион",   callback_data="auction_list"),
    ])
    # Категории товаров
    buttons.append([
        InlineKeyboardButton(
    ...
    buttons.append([InlineKeyboardButton(text="« Назад", callback_data=f"mkt_sort:{sort}:all:{page_str}")])
...
async def callback_daily(callback: CallbackQuery):
    ...
    if success:
        ...
        advance_quest(callback.from_user.id, "daily_bonus")
        return
    else:
...
async def callback_buy_gpu(callback: CallbackQuery):
    ...
    detail_text = (
        ...
        f" ⤷💰 Тон за майнинг: +{int(gpu['hash_rate'])}\n\n"
...
async def callback_buy_cooler(callback: CallbackQuery):
    ...
    if not cooler_data:
        await callback.answer("❌ Кулер не найден!", show_alert=True)
        return
...
async def callback_buy_asic(callback: CallbackQuery):
    ...
    detail_text = (
        ...
        f"📊 <b>Характеристики:</b>\n"
        f" ⤷💰 Тон за майнинг: +{int(asic['hash_rate'])}\n\n"
        f"💰 <b>Твой баланс ››</b> {user_balance} тон"
    )
...
async def callback_buy_rig(callback: CallbackQuery):
    ...
    detail_text = (
        ...
        f" ⤷💨 Слотов для кулеров ›› {rig['cooler_slots']}\n"
...
async def callback_my_rigs(callback: CallbackQuery):
    ...
    extra_buttons = []
    if asics:
        extra_buttons.append([InlineKeyboardButton(text=f"💽 ASIC-майнеры ({len(asics)} шт.)", callback_data="my_asics")])

    if rigs:
        text += f"<b>💿 GPU-риги ({len(rigs)} шт.):</b>\nВыбери риг для настройки:"
        rigs_kb = get_rigs_keyboard(callback.from_user.id, rigs, 0)
        # Добавляем кнопку ASIC перед кнопками рига
        rigs_kb.inline_keyboard = extra_buttons + rigs_kb.inline_keyboard
        try:
            await callback.message.edit_text(text, reply_markup=rigs_kb, parse_mode="HTML")
        except Exception:
            await callback.message.answer(text, reply_markup=rigs_kb, parse_mode="HTML")
    else:
        if not asics:
            text = "💿 <b>У тебя пока нет майнеров!</b>\n\nКупи ASIC или GPU-риг в магазине."
        else:
            text += "<i>GPU-ригов нет. Купи в /shop</i>"
        keyboard = InlineKeyboardMarkup(inline_keyboard=extra_buttons + [
            [InlineKeyboardButton(text="🛒 Магазин", callback_data="shop_menu")],
...
@dp.callback_query(F.data == "my_asics")
async def callback_my_asics(callback: CallbackQuery):
    if await check_ban(callback.from_user.id):
        await send_ban_message(callback)
        return
    await callback.answer()
    ...
    if not asics:
        try:
            await callback.message.edit_text("💽 <b>У тебя нет ASIC-майнеров</b>", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="« Назад", callback_data="my_rigs")]]), parse_mode="HTML")
        except Exception:
            await callback.message.answer("💽 <b>У тебя нет ASIC-майнеров</b>", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="« Назад", callback_data="my_rigs")]]), parse_mode="HTML")
        return
    lines = ["💽 <b>Твои ASIC-майнеры</b>\n"]
    for asic in asics:
        lines.append(f"✅ <b>{asic[3]}</b> — {asic[4]} HM/s")
    text = "\n".join(lines)
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="« Назад", callback_data="my_rigs")]])
    try:
        await callback.message.edit_text(text, reply_markup=kb, parse_mode="HTML")
    except Exception:
        await callback.message.answer(text, reply_markup=kb, parse_mode="HTML")
...
async def callback_rig_cooler_slot(callback: CallbackQuery):
    ...
    text = f"Слот для кулера #{slot_index + 1}\n\n"
    cooler_id = None
    is_broken = False
    ...
    if not is_empty:
        ...
        if cooler:
            try:
                ...
                if is_broken:
                    text += f"\n⚠️ Кулер сломан! Требуется ремонт."
            except (ValueError, TypeError):
                text += "❌ Ошибка: данные повреждены"
        else:
            text += "❌ Ошибка: кулер не найден"
    else:
        text += "Слот пуст. Нажми 'Установить кулер', чтобы выбрать охлаждение."
...
async def callback_install_cooler(callback: CallbackQuery):
    ...
    if len(parts) == 4:
        ...
        if not coolers:
            await callback.answer("❌ У тебя нет свободных кулеров!", show_alert=True)
            return
        
        text = f"Выбери кулер для установки в слот #{slot_index + 1}:"
...
async def callback_repair_cooler(callback: CallbackQuery):
    ...
    if not cooler or cooler[1] != callback.from_user.id:
        await callback.answer("❌ Этот кулер тебе не принадлежит!", show_alert=True)
        return
    
    if cooler[5] >= 100:
        await callback.answer("✅ Кулер уже в идеальном состоянии!", show_alert=True)
        return
    ...
    await callback.answer("✅ Кулер отремонтирован!", show_alert=False)
...
async def callback_top_category(callback: CallbackQuery):
    ...
    titles = {
        ...
        "streak":   "🔥 Топ по стрикам",
        "duels":    "⚔️ Топ по дуэлям",
    }
    ...
    if not results:
        ...
        for i, res in enumerate(results, 1):
            ...
            if len(res) >= 4:
                ...
                # Формируем имя (показываем ник/имя, не @username)
                name_display = escape_html(first_name or username or f"Игрок {i}")
                
                icon = get_top_icon(i)
                
                # Получаем значение в зависимости от категории
                if category == "gems":
                    value = f"{fmt(res[3])} тон"
                elif category == "level":
                    if len(res) >= 5:
                    ...
                    value = f"{res[3]} побед ⚔️"
                else:
                    value = str(res[3])
                
                text += f"{icon} {name_display} — {value}\n"
        
        text += "</blockquote>"
    
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="« К выбору топа", callback_data="top_menu")]
    ])
...
async def callback_track_referrals(callback: CallbackQuery):
    ...
    if not referrals:
        ...
        text += f"├ 👤 Рефералов: <b>{fmt(referrals_count)}</b>\n"
        text += f"├ 💰 Получено тон: <b>{total_ton_bonus}</b>\n"
        text += f"└ ✨ Получено опыта: <b>{total_exp_bonus}</b>"
    
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="« Назад", callback_data="referrals_page")]
    ])
...
async def callback_my_storage(callback: CallbackQuery):
    """Показывает склад с компонентами, видеокартами и кулерами"""
    if await check_ban(callback.from_user.id):
        await send_ban_message(callback)
        return
    ...
    # Кулеры
    if coolers:
        text += f"<b>💨 Кулеры ({len(coolers)} шт.):</b>\n"
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🖼 Все видеокарты", callback_data="my_gpus"),
         InlineKeyboardButton(text="💨 Все кулеры", callback_data="my_coolers")],
...
async def callback_my_coolers(callback: CallbackQuery):
    """Показывает все кулеры пользователя"""
    if await check_ban(callback.from_user.id):
        await send_ban_message(callback)
        return
    await callback.answer()
    try:
        ...
        # Исключаем кулеры на барахолке (rig_id = -1)
        ...
        text = "💨 <b>Твои кулеры</b>\n\nВыбери кулер для просмотра:"
        await callback.message.edit_text(
            text,
            reply_markup=get_mycards_keyboard(callback.from_user.id, items, 0),
            parse_mode="HTML"
...
def get_gpu_detail_keyboard(gpu_id, is_broken, is_installed, rig_id):
    ...
    buttons.append([InlineKeyboardButton(text="« Назад", callback_data="my_gpus")])
    return InlineKeyboardMarkup(inline_keyboard=buttons)


def get_cooler_detail_keyboard(cooler_id, is_broken, is_installed, rig_id):
    """Клавиатура для детального просмотра кулера"""
    buttons = []
    if is_broken:
    ...
    buttons.append([InlineKeyboardButton(text="« Назад", callback_data="my_coolers")])
    return InlineKeyboardMarkup(inline_keyboard=buttons)
...
async def callback_cooler_detail(callback: CallbackQuery):
    ...
    cooler = db.get_cooler_by_id(cooler_id)
    if not cooler:
        await callback.message.edit_text("❌ Кулер не найден!")
        return
    ...
    detail_text = (
        ...
        f"━━━━━━━━━━━━━━━━━━━━\n\n"
        f"📊 <b>Характеристики:</b>\n"
        f" ⤷💨 Мощность охлаждения ›› {cooler[4]}\n"
        f" ⤷🔧 Статус ›› {status}\n"
    )
...
async def callback_help_equipment(callback: CallbackQuery):
    ...
    text = (
        ...
        "• 💨 Кулеры - уменьшают износ видеокарт\n"
...
async def callback_help_repair(callback: CallbackQuery):
    ...
    text = (
        ...
        f"• Кулеры уменьшают износ видеокарт\n\n"
...
async def callback_profile_settings(callback: CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🎁 Активировать промо", callback_data="redeem_promo")],
    ...
    if len(new_nick) > 32:
        await message.answer(
            ...
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="« Назад в настройки", callback_data="profile_settings")]
            ])
        )
        return
...
async def q_edit_gender(callback: CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="« Назад", callback_data="profile_questionnaire")]
    ])
    await callback.message.edit_text(
...
async def process_transfer_amount(message: Message, state: FSMContext):
    ...
    if success:
        ...
        await message.answer(
            ...
            f"<blockquote>"
            f"💸 Отправлено: <b>{fmt(amount)} тон</b>\n"
...
async def callback_my_transfers(callback: CallbackQuery):
    ...
    for t in transfers:
        ...
        if from_id == user_id:
            # Исходящий
            text += f"📤 <b>-{fmt(amount)} тон</b> → {escape_html(to_name or '?')}\n"
        else:
            # Входящий
            text += f"📥 <b>+{fmt(amount)} тон</b> ← {escape_html(from_name or '?')}\n"
...
async def callback_change_profile_photo(callback: CallbackQuery, state: FSMContext):
    ...
    buttons.append([InlineKeyboardButton(text="« Назад в настройки", callback_data="profile_settings")])
...
async def show_user_profile(message_or_callback, user_id, is_own=True):
    ...
    priv_name, priv_until = db.get_user_privilege(user_id)
    priv_cfg = config.PRIVILEGES.get(priv_name, config.PRIVILEGES["player"])
    until_str = ""
    if priv_name != "player" and priv_until:
        try:
            until_str = datetime.fromisoformat(priv_until).strftime("%d.%m.%Y")
        except Exception:
            until_str = "?"

    priv_line_str = f"{priv_cfg['icon']} {priv_cfg['name']}" + (f" (до {until_str})" if until_str else "")
...
async def show_level_info(message_or_callback, user_id):
    ...
    if current_level < config.MAX_LEVEL and ton_cost and exp_cost:
        text = (
            ...
            f"<blockquote>"
            f"👤 {escape_html(nick)}\n\n"
            f"💰 Тон ›› {fmt(current_ton)} | {fmt(ton_cost)}\n"
...
async def inline_query_handler(inline_query: InlineQuery):
    ...
    try:
        ...
        # ── РЕЖИМ: дуэль — @бот ЧИСЛО (одно число = дуэль)
        #    @bot 10000  → вызов на дуэль со ставкой 10000
        if len(parts) == 1 and parts[0].isdigit():
            try:
                bet = int(parts[0])
            except (ValueError, IndexError):
                bet = 100
            bet = max(10, min(bet, 100_000))
            cu2 = db.conn.cursor()
            cu2.execute("SELECT gems, first_name, custom_id FROM users WHERE user_id=?", (user_id,))
            row2 = cu2.fetchone()
            if row2 and int(row2[0]) >= bet:
                challenger_name = row2[1] or "Игрок"
                challenger_mid  = row2[2]
                duel_text = (
                    f"⚔️ <b>{escape_html(challenger_name)} вызывает на дуэль!</b>\n\n"
                    f"<blockquote>"
                    f"💰 Ставка: {fmt(bet)} тон\n"
                    f"🎲 Каждый бросает кубик — у кого больше, тот забирает всё\n"
                    f"Победитель получает: {fmt(bet * 1.9)} тон"
                    f"</blockquote>\n\n"
                    f"Нажми кнопку чтобы принять вызов!"
                )
                duel_kb = InlineKeyboardMarkup(inline_keyboard=[[
                    InlineKeyboardButton(
                        text=f"⚔️ Принять дуэль ({fmt(bet)} тон)",
                        url=f"https://t.me/{bot_username}?start=duel_{user_id}_{bet}"
                    )
                ]])
                results.append(art(
                    f"duel_{bet}",
                    f"⚔️ Вызов на дуэль — ставка {fmt(bet)} тон",
                    f"Баланс: {fmt(row2[0])} тон | Пошли вызов в чат!",
                    duel_text,
                    markup=duel_kb
                ))
            else:
                bal = int(row2[0]) if row2 else 0
                results.append(art(
                    "duel_no_bal",
                    f"❌ Недостаточно тон для дуэли (нужно {fmt(bet)}, есть {fmt(bal)})",
                    "Сначала намайни тоны",
                    f"❌ Недостаточно тон для дуэли.\nНужно: {fmt(bet)}, есть: {bal}"
                ))
                ...
                if amount > 0 and activations >= 1:
                    ...
                    if balance >= total_cost:
                        ...
                        if ok:
                            ...
                            check_text = (
                                f"🧾 <b>Чек #{code}</b>\n\n"
                                f"<blockquote>"
                                f"💰 Сумма: {fmt(amount)} тон за активацию\n"
                                f"🔁 Активаций: {fmt(activations)}\n"
                            ...
                            btn_check = InlineKeyboardMarkup(inline_keyboard=[[
                                InlineKeyboardButton(
                                    text=f"💰 Получить {fmt(amount)} тон",
                                    url=f"https://t.me/{bot_username}?start=check_{code}"
                                )
                            ]])
                            results.append(art(
                                f"check_{code}",
                                f"🧾 Создать чек — дать {fmt(amount)} тон × {fmt(activations)}",
                                f"Чек {for_who} | баланс: {fmt(balance)} тон",
                                check_text,
                                markup=btn_check
                            ))
                        else:
                        ...
                        results.append(art(
                            "no_bal_check",
                            f"🧾 Создать чек — недостаточно тон (нужно {fmt(total_cost)}, у тебя {balance})",
                            "Пополни баланс через майнинг",
                            f"❌ Недостаточно тон для чека.\nНужно: {fmt(total_cost)} тон, у тебя: {fmt(balance)} тон"
                        ))
                    ...
                    if ok2:
                        ...
                        invoice_text = (
                            f"📄 <b>Счёт #{code2}</b>\n\n"
                            f"<blockquote>"
                            f"💸 Сумма к оплате: {fmt(amount)} тон\n"
                            f"🔁 Активаций: {fmt(activations)}\n"
                            + (f"👤 Для: @{escape_html(target_username)}\n" if target_username else "👥 Публичный\n")
                            + f"</blockquote>\n\n"
                        ...
                        btn_invoice = InlineKeyboardMarkup(inline_keyboard=[[
                            InlineKeyboardButton(
                                text=f"💳 Оплатить {fmt(amount)} тон",
                                url=f"https://t.me/{bot_username}?start=check_{code2}"
                            )
                        ]])
                        results.append(art(
                            f"invoice_{code2}",
                            f"📄 Создать счёт — получить {fmt(amount)} тон × {fmt(activations)}",
        ...
        stats_text = (
            ...
            f"│  └ Текущий онлайн ›› {stats['current_online']}\n"
            f"│\n"
            f"├ 💰 Тон у игроков ›› {fmt(stats['total_ton'])}\n"
            f"│  └ Тон на чел ›› {fmt(stats['avg_ton'])}\n"
        ...
        if user:
            try:
                ...
                results.append(art(
                    "my_profile",
                    "👤 Мой профиль",
                    f"Тон: {fmt(user[4])} | HM/s: {float(user[5]):.1f} | Ур. {user[10]}",
                    my_text
                ))
            except Exception as e:
            ...
            hint = (
                ...
                f"💰 Твой баланс: {fmt(balance)} тон"
                f"</blockquote>"
            )
            results.append(art(
                "help_checks",
                f"🧾 Чек / 📄 Счёт — напиши @{bot_username} 100 5",
                f"Пример: @{bot_username} 100 5  |  баланс: {fmt(balance)} тон",
                hint
            ))
            ...
            for ch in cc.fetchall():
                ...
                btn_text = f"💰 Получить {fmt(amount)} тон" if ch_type == "check" else f"💳 Оплатить {fmt(amount)} тон"
                check_text = (
                    f"{icon} <b>{type_name_ch} #{code}</b>\n\n"
                    f"<blockquote>"
                    f"💰 Сумма: {fmt(amount)} тон за активацию\n"
                    f"🔁 Активаций: {fmt(act_left)}/{fmt(act_total)}\n"
                ...
                results.append(art(
                    f"check_{code}",
                    f"{icon} {type_name_ch} #{code} — {fmt(amount)} тон × {fmt(act_left)} ост.",
                    f"Активаций: {fmt(act_left)}/{fmt(act_total)}" + (f" | @{tg_un}" if tg_un else " | Публичный"),
                    check_text,
                    markup=btn_markup
                ))
...
async def _ask_check_amount(msg, edit=False):
    ...
    text = (
        ...
        f"Твой баланс: <b>{fmt(balance)} тон</b></blockquote>"
    )
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="❌ Отмена", callback_data="checks_menu")]
    ])
...
async def _finalize_check(msg_or_cb, state: FSMContext, user_id):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="« В меню чеков", callback_data="checks_menu")]
    ])
...
async def callback_my_checks_list(callback: CallbackQuery):
    ...
    for ch in checks:
        ...
        icon = "🧾" if ch_type == "check" else "📄"
        text += f"{status} {icon} <b>#{code}</b> — {fmt(amount)} тон × {fmt(act_left)}/{fmt(act_total)}\n"
        if is_active:
            buttons.append([InlineKeyboardButton(
                text=f"{icon} #{code} ({fmt(act_left)} ост.)",
                callback_data=f"check_detail_{ch_id}"
            )])
    
    text += "</blockquote>"
    buttons.append([InlineKeyboardButton(text="« Назад", callback_data="checks_menu")])
...
async def callback_check_detail(callback: CallbackQuery):
    ...
    text = (
        ...
        f"<blockquote>"
        f"💰 Сумма: {fmt(amount)} тон\n"
        f"🔁 Активаций: {fmt(act_left)}/{fmt(act_total)}\n"
        + (f"👤 Для: @{tg_un}\n" if tg_un else "👥 Публичный\n")
        + (f"💬 {escape_html(comment)}\n" if comment else "")
        + f"📊 Статус: {status}\n"
        + (f"💸 Остаток: {fmt(amount * act_left)} тон\n" if ch_type == "check" and is_active else "")
        + f"</blockquote>\n\n"
    ...
    buttons.append([InlineKeyboardButton(text="« Назад к списку", callback_data="my_checks_list")])
...
async def callback_cancel_check(callback: CallbackQuery):
    ...
    if success:
        await callback.answer(f"✅ Чек отменён! Возвращено: {fmt(result)} тон", show_alert=True)
...
async def cmd_userinfo(message: Message):
    ...
    text = (
        ...
        f"├ 💰 Тон: {fmt(user[4])}\n"
        f"├ ⚡ HM/s: {user[5]:.1f}\n"
        f"├ 💎 Всего намайнено: {fmt(user[14])}\n"
        f"│\n"
        f"├ 🖥 ASIC: {len(asics)} | Риги: {len(rigs)} | GPU: {len(gpus)} | Кулеры: {len(coolers)}\n"
...
async def cmd_referrals_admin(message: Message):
    ...
    if not refs:
        ...
        for name, uname, mid, created, gems, level in refs:
            ...
            text += f" | Ур.{level} | {fmt(gems)} тон | {dt}\n"
    text += "</blockquote>"
    await message.answer(text, parse_mode="HTML")
...
async def cmd_transfers_admin(message: Message):
    ...
    for amount, created_at, comment, from_name, to_name, from_id, to_id in transfers:
        dt = created_at[:16] if created_at else "?"
        if from_id == uid:
            text += f"📤 <b>-{fmt(amount)}</b> → {escape_html(to_name or '?')} [{dt}]\n"
        else:
            text += f"📥 <b>+{fmt(amount)}</b> ← {escape_html(from_name or '?')} [{dt}]\n"
        if comment:
            text += f"   💬 {escape_html(comment)}\n"
    text += "</blockquote>"
    await message.answer(text, parse_mode="HTML")
...
async def cmd_userchecks(message: Message):
    ...
    for code, ch_type, amount, act_left, act_total, target_un, is_active, created_at in checks:
        ...
        text += f"{status}{icon} <code>{code}</code> | {fmt(amount)} тон × {fmt(act_left)}/{fmt(act_total)}"
        if target_un:
            text += f" | @{target_un}"
        text += f" | {dt}\n"
    text += "</blockquote>"
...
async def cmd_giveton(message: Message):
    ...
    db.conn.commit()
    db.log_admin_action(message.from_user.id, "giveton", uid, f"Выдано {fmt(amount)} тон")
    ...
    await message.answer(
        ...
        f"<blockquote>"
        f"👤 {escape_html(target_name)} (MID: {user[1]})\n"
        f"💰 Изменение: {sign}{fmt(amount)} тон\n"
    ...
    try:
        await bot.send_message(
            uid,
            f"{'💰' if amount >= 0 else '💸'} <b>{'Получено' if amount >= 0 else 'Снято'}: {sign}{fmt(amount)} тон</b>\n"
...
async def cmd_givebox(message: Message):
    ...
    db.conn.commit()
    db.log_admin_action(message.from_user.id, "givebox", uid, f"Выдано {fmt(count)} боксов")
    ...
    try:
        await bot.send_message(
            uid,
            f"🎁 <b>Тебе выдали {fmt(count)} 📦 боксов!</b>\n"
...
async def cmd_createpromo(message: Message):
    """/createpromo <код> <тоны> <опыт> <боксы> <звёзды> [детали=0] [uses=1]"""
    if not is_admin(message.from_user.id):
        await message.answer("❌ Нет прав!")
        return
    args = message.text.split()
    if len(args) < 6:
        await message.answer(
            "❌ Использование:\n"
            "<code>/createpromo &lt;код&gt; &lt;тоны&gt; &lt;опыт&gt; &lt;боксы&gt; &lt;звёзды&gt; [детали] [использований]</code>\n\n"
            "Пример: <code>/createpromo ГРУЗ200 200 200 2 20</code>\n"
            "💰 тоны · ⭐ опыт · 📦 боксы · ✨ звёзды · 🧩 детали",
            parse_mode="HTML"
        )
        return
    code = args[1].upper()
    ...
    boxes    = _int(args[4])
    stars    = _int(args[5])
    parts    = _int(args[6]) if len(args) > 6 else 0
    max_uses = _int(args[7], 1) if len(args) > 7 else 1
    if max_uses < 1:
        max_uses = 1
    db.cursor.execute(
        "INSERT INTO resource_promos (code, gems, exp, boxes, stars, parts, max_uses, uses, created_by) "
        "VALUES (?,?,?,?,?,?,?,0,?) "
        "ON CONFLICT(code) DO UPDATE SET gems=excluded.gems, exp=excluded.exp, boxes=excluded.boxes, "
        "stars=excluded.stars, parts=excluded.parts, max_uses=excluded.max_uses, uses=0, created_by=excluded.created_by",
        (code, gems, exp, boxes, stars, parts, max_uses, message.from_user.id)
    )
    db.cursor.execute("DELETE FROM resource_promo_uses WHERE code=?", (code,))
    db.conn.commit()
    lines = [f"✅ Промокод <b>{code}</b> создан/обновлён!", f"Макс. использований: {max_uses}"]
    if gems:  lines.append(f"💰 Тоны: +{gems}")
    if exp:   lines.append(f"⭐ Опыт: +{exp}")
    if boxes: lines.append(f"📦 Боксы: +{boxes}")
    if stars: lines.append(f"✨ Звёзды: +{stars}")
    if parts: lines.append(f"🧩 Детали GPU: +{parts}")
...
async def cmd_deletepromo(message: Message):
    ...
    args = message.text.split()
    if len(args) < 2:
        await message.answer("❌ Использование: <code>/deletepromo &lt;код&gt;</code>", parse_mode="HTML")
        return
    code = args[1].upper()
    db.cursor.execute("DELETE FROM resource_promos WHERE code=?", (code,))
    db.cursor.execute("DELETE FROM resource_promo_uses WHERE code=?", (code,))
    db.conn.commit()
    if db.cursor.rowcount is not None:
...
async def callback_redeem_promo(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    await state.set_state("waiting_promo_code")
    await callback.message.answer(
        "🎟 Введи промокод:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="❌ Отмена", callback_data="donate_menu")]])
    )
...
async def handle_promo_code_input(message: Message, state: FSMContext):
    ...
    promo = db.cursor.execute(
        "SELECT gems, exp, boxes, stars, parts, max_uses, uses FROM resource_promos WHERE code=?", (code,)
    ).fetchone()
    if not promo:
        await message.answer("❌ Промокод не найден.")
        return
    gems, exp_reward, boxes, stars, parts_count, max_uses, uses = promo
    if uses >= max_uses:
        await message.answer("❌ Промокод уже использован максимальное количество раз.")
        return
    try:
        db.cursor.execute(
            "INSERT INTO resource_promo_uses (code, user_id) VALUES (?,?)",
            (code, message.from_user.id)
        )
    except Exception:
        await message.answer("❌ Ты уже использовал этот промокод.")
        return
    db.cursor.execute("UPDATE resource_promos SET uses=uses+1 WHERE code=?", (code,))
    if gems:
        db.cursor.execute("UPDATE users SET gems=gems+? WHERE user_id=?", (gems, message.from_user.id))
    if exp_reward:
        db.cursor.execute("UPDATE users SET experience=experience+? WHERE user_id=?", (exp_reward, message.from_user.id))
    if boxes:
        db.cursor.execute("UPDATE users SET boxes=boxes+? WHERE user_id=?", (boxes, message.from_user.id))
    if stars:
        db.cursor.execute("UPDATE users SET stars_balance=COALESCE(stars_balance,0)+? WHERE user_id=?", (stars, message.from_user.id))
    if parts_count:
        _part_types = ["🧩 Чип", "🧩 Вентилятор", "🧩 Кабель", "🧩 Радиатор", "🧩 Микросхема"]
        for _ in range(parts_count):
            db.add_component(message.from_user.id, random.choice(_part_types), 1)
    db.conn.commit()
    reward_lines = []
    if gems:        reward_lines.append(f"  💰 +{gems} тонов")
    if exp_reward:  reward_lines.append(f"  ⭐ +{exp_reward} опыта")
    if boxes:       reward_lines.append(f"  📦 +{boxes} боксов")
    if stars:       reward_lines.append(f"  ✨ +{stars} звёзд")
    if parts_count: reward_lines.append(f"  🧩 +{parts_count} деталей GPU")
    rewards_text = "\n".join(reward_lines) if reward_lines else "  (нет наград)"
    await message.answer(
        f"🎉 Промокод <b>{code}</b> активирован!\n\n"
        f"<b>Твои награды:</b>\n{rewards_text}",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🪙 Донат", callback_data="donate_menu")]])
    )


@dp.message(Command("giveexp"))
async def cmd_giveexp(message: Message):
    """/giveexp <MID|@user> <кол-во> — выдать опыт игроку"""
    if not is_admin(message.from_user.id):
        await message.answer("❌ Нет прав!")
        return
    args = message.text.split()
    if len(args) < 3 or not args[2].lstrip("-").isdigit():
        await message.answer("❌ Использование: <code>/giveexp &lt;MID|@user&gt; &lt;кол-во&gt;</code>", parse_mode="HTML")
        return
    user = _find_user_by_arg(args[1])
    if not user:
        await message.answer("❌ Игрок не найден!")
    ...
    amount = int(args[2])
    target_name = user[3] or user[2] or f"MID:{user[1]}"
    db.cursor.execute("UPDATE users SET experience=experience+? WHERE user_id=?", (amount, uid))
    db.conn.commit()
    db.log_admin_action(message.from_user.id, "giveexp", uid, f"Выдано {amount} exp")
    await message.answer(
        f"✅ <b>Опыт {'выдан' if amount >= 0 else 'снят'}!</b>\n"
        f"👤 {escape_html(target_name)} (MID: {user[1]})\n"
        f"⭐ Изменение: {'+' if amount>=0 else ''}{amount} exp",
        parse_mode="HTML"
    )
    try:
        await bot.send_message(uid, f"⭐ Вам {'выдано' if amount>=0 else 'снято'}: {'+' if amount>=0 else ''}{amount} опыта от администратора!")
    except Exception:
        pass
...
async def cmd_admhelp(message: Message):
    ...
    text = (
        ...
        "<blockquote>"
        "<b>👤 Игроки:</b>\n"
        "/userinfo &lt;MID|@user&gt; — инфо об игроке\n"
        "/userlogs &lt;MID&gt; [N] — логи действий\n"
        "/referrals &lt;MID&gt; — рефералы игрока\n"
        "/transfers &lt;MID&gt; [N] — история переводов\n"
        "/userchecks &lt;MID&gt; — чеки и счета\n"
        "\n"
        "<b>💰 Экономика:</b>\n"
        "/giveton &lt;MID|@user&gt; &lt;сумма&gt; — тоны\n"
        "/givestars &lt;MID|@user&gt; &lt;кол-во&gt; — звёзды\n"
        "/giveexp &lt;MID|@user&gt; &lt;кол-во&gt; — опыт\n"
        "/givebox &lt;MID|@user&gt; &lt;кол-во&gt; — боксы\n"
        "/pay &lt;MID&gt; &lt;сумма&gt; — перевод тон\n"
        "/take &lt;MID&gt; &lt;сумма&gt; — снять тоны\n"
        "\n"
        "<b>🔨 Модерация:</b>\n"
        ...
        "\n"
        "<b>📋 Логи:</b>\n"
        "/adminlogs [N] — последние действия\n"
        "/userlogs &lt;MID&gt; [N] — логи игрока\n"
        "\n"
        "<b>🛒 Магазин:</b>\n"
        "/skl — управление складом\n"
        "\n"
        "<b>🎟 Промокоды:</b>\n"
        "/createpromo &lt;код&gt; &lt;тоны&gt; &lt;опыт&gt; &lt;боксы&gt; &lt;звёзды&gt; [детали] [uses]\n"
        "  Пример: /createpromo ЛЕТО 200 200 2 20\n"
        "/deletepromo &lt;код&gt; — удалить промокод"
        "</blockquote>"
    )
    await message.answer(text, parse_mode="HTML")
...
async def cmd_openbox(message: Message):
    ...
    for box in boxes_to_open:
        ...
        if success:
            ...
            # Ищем бонусные предметы (кулеры, привилегии)
    ...
    text = (
        ...
        f"<blockquote>"
        f"💰 Тон получено: <b>+{fmt(total_ton)}</b>\n"
        f"✨ Опыта получено: <b>+{fmt(total_exp)}</b>\n"
    )
    if components:
...
async def callback_craft_view(callback: CallbackQuery):
    ...
    buttons.append([InlineKeyboardButton(text="« К рецептам", callback_data="craft_menu")])
...
def market_sort_keyboard(current_sort, item_type_filter, page):
    ...
    filter_labels = {"all": "Все товары", "gpu": "GPU", "cooler": "Кулеры", "component": "Компоненты"}
    buttons = [
        [InlineKeyboardButton(
...
def build_market_text_and_keyboard(user_id, sort, item_filter, page):
    ...
    filter_labels = {"all": "Все товары", "gpu": "Видеокарты", "cooler": "Кулеры", "component": "Компоненты"}
    sort_labels = {
...
async def callback_market(callback: CallbackQuery, state: FSMContext):
    """Барахолка — сразу открываем список."""
    await callback.answer()
    await state.clear()
...
@dp.callback_query(F.data == "market_flea")
async def callback_market_flea(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    await state.clear()
    text, kb = build_market_text_and_keyboard(callback.from_user.id, "price_asc", "all", 0)
    await callback.message.edit_text(text, reply_markup=kb, parse_mode="HTML")
...
async def callback_mkt_my_listings(callback: CallbackQuery):
    ...
    if not listings:
        text += "У тебя нет активных лотов.\n\nВыставь видеокарту или кулер со склада!"
    else:
        for lst in listings:
            lid, seller_id, itype, iid, iname, hr, cp, wear, price, is_crafted, listed_at, expires_at, is_active = lst
            craft_tag = " ⚒️" if is_crafted else ""
    ...
    buttons.append([InlineKeyboardButton(text="« Барахолка", callback_data="market")])
    await callback.message.edit_text(
        text, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons), parse_mode="HTML"
    )
...
async def callback_mkt_sell_choose(callback: CallbackQuery):
    """Показываем выбор категории: GPU / Кулер"""
    await callback.answer()
    buttons = [
        [InlineKeyboardButton(text="🖼 Видеокарты (GPU)", callback_data="mkt_inv_gpu")],
        [InlineKeyboardButton(text="💨 Кулеры",          callback_data="mkt_inv_cooler")],
        [InlineKeyboardButton(text="« Назад",             callback_data="mkt_my_listings")],
    ]
    await callback.message.edit_text(
        "🏪 <b>Выставить товар</b>\n\nВыбери категорию:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons),
...
async def callback_mkt_inv_gpu(callback: CallbackQuery):
    ...
    buttons.append([InlineKeyboardButton(text="« Назад", callback_data="mkt_sell_choose")])
...
async def callback_mkt_inv_cooler(callback: CallbackQuery):
    """Список свободных кулеров для продажи"""
    await callback.answer()
    c = db.conn.cursor()
    c.execute("""
    ...
    if not rows:
        await callback.answer("❌ Нет свободных кулеров для продажи", show_alert=True)
        return
...
async def callback_mkt_sell_cooler(callback: CallbackQuery, state: FSMContext):
    """Нажали 'Продать на барахолке' для кулера"""
    await callback.answer()
    ...
    row = c.fetchone()
    if not row:
        await callback.answer("❌ Кулер недоступен", show_alert=True)
        return
...
async def callback_mkt_quick_sell_cooler(callback: CallbackQuery):
    """Быстрая продажа кулера за 30% без ввода цены"""
    await callback.answer()
    parts = callback.data.split(":")
...
# ╔══════════════════════════════════════════════════════════════╗
# ║              ПОДАРКИ ОТ АДМИНИСТРАЦИИ                        ║
# ╚══════════════════════════════════════════════════════════════╝




ADMIN_GIFT_RESOURCES = {
    "ton":   ("💰 Тоны",    "тон"),
    "stars": ("⭐ Звёзды",  "звёзд"),
    "box":   ("📦 Боксы",   "боксов"),
    "exp":   ("✨ Опыт",    "единиц опыта"),
}


def _admin_gift_resource_kb():
    rows = []
    for key, (label, _) in ADMIN_GIFT_RESOURCES.items():
        rows.append([InlineKeyboardButton(text=label, callback_data=f"ag_res:{key}")])
    rows.append([InlineKeyboardButton(text="❌ Отмена", callback_data="ag_cancel")])
    return InlineKeyboardMarkup(inline_keyboard=rows)




@dp.message(Command("botbalance"))
async def cmd_botbalance(message: Message):
    """Показывает баланс бота (комиссии с дуэлей и барахолки)."""
    if not is_admin(message.from_user.id):
        return
    try:
        c2 = db.conn.cursor()
        c2.execute("SELECT balance FROM bot_balance WHERE id=1")
        row = c2.fetchone()
        bal = row[0] if row else 0
        await message.answer(
            f"🤖 <b>Баланс бота</b>\n\n"
            f"💰 Накоплено: <b>{fmt(bal)}</b> тон\n\n"
            f"<i>Источники: 5% с дуэлей, 5% с барахолки, 5% с проигрышей в играх</i>",
            parse_mode="HTML"
        )
    except Exception as e:
...
@dp.message(Command("botwithdraw"))
async def cmd_botwithdraw(message: Message):
    """Снять тоны с баланса бота на свой счёт."""
    if not is_admin(message.from_user.id):
        return
    args = message.text.split()
    if len(args) < 2 or not args[1].isdigit():
        await message.answer("Использование: /botwithdraw СУММА")
        return
    amount = int(args[1])
    try:
        c2 = db.conn.cursor()
        ...
        bal = row[0] if row else 0
        if amount > bal:
            await message.answer(f"❌ Недостаточно средств (есть {fmt(bal)} тон)")
            return
        db.cursor.execute("UPDATE bot_balance SET balance=balance-? WHERE id=1", (amount,))
        db.cursor.execute("UPDATE users SET gems=gems+? WHERE user_id=?", (amount, message.from_user.id))
        db.conn.commit()
        await message.answer(f"✅ Снято {fmt(amount)} тон с баланса бота на ваш счёт.")
    except Exception as e:
        await message.answer(f"❌ Ошибка: {e}")


@dp.message(Command("admingift"))
async def cmd_admingift(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
    await state.clear()
    await state.set_state(AdminGiftStates.choosing_resource)
    await message.answer(
        "🎁 <b>Подарок от администрации</b>\n\nВыбери тип ресурса:",
        reply_markup=_admin_gift_resource_kb(),
        parse_mode="HTML"
    )


@dp.callback_query(F.data.startswith("ag_res:"))
async def callback_ag_res(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id):
        return
    await callback.answer()
    res_key = callback.data.split(":")[1]
    label, unit = ADMIN_GIFT_RESOURCES[res_key]
    await state.update_data(resource=res_key, unit=unit, label=label)
    await state.set_state(AdminGiftStates.entering_amount)
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="❌ Отмена", callback_data="ag_cancel")]
    ])
    await callback.message.edit_text(
        f"🎁 Ресурс: <b>{label}</b>\n\nВведи количество {unit} для каждого игрока:",
        reply_markup=kb, parse_mode="HTML"
    )


@dp.message(AdminGiftStates.entering_amount)
async def process_ag_amount(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
    ...
    await state.set_state(AdminGiftStates.entering_count)
    data = await state.get_data()
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🌍 Всем игрокам", callback_data="ag_count:all")],
        [InlineKeyboardButton(text="🕐 Активным за 24ч", callback_data="ag_count:active")],
        [InlineKeyboardButton(text="✏️ Конкретный MID", callback_data="ag_count:mid")],
        [InlineKeyboardButton(text="❌ Отмена", callback_data="ag_cancel")],
    ])
    await message.answer(
        f"🎁 {data['label']}: <b>{amount}</b> {data['unit']}\n\nКому отправить?",
        reply_markup=kb, parse_mode="HTML"
    )


@dp.callback_query(F.data.startswith("ag_count:"))
async def callback_ag_count(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id):
        return
    await callback.answer()
    mode = callback.data.split(":")[1]
    if mode == "mid":
        await state.update_data(mode="mid")
        await state.set_state(AdminGiftStates.confirming)
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="❌ Отмена", callback_data="ag_cancel")]
        ])
        await callback.message.edit_text(
            "✏️ Введи MID игрока (можно несколько через пробел):",
            reply_markup=kb
        )
        return
    await state.update_data(mode=mode)
    data = await state.get_data()
    # Считаем сколько игроков
    c2 = db.conn.cursor()
    if mode == "all":
        c2.execute("SELECT COUNT(*) FROM users WHERE banned=0")
    else:
        c2.execute("SELECT COUNT(*) FROM users WHERE banned=0 AND last_mining > datetime('now','-1 day')")
    cnt = c2.fetchone()[0]
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"✅ Отправить ({cnt} игроков)", callback_data="ag_send:confirmed")],
        [InlineKeyboardButton(text="❌ Отмена", callback_data="ag_cancel")],
    ])
    target_label = "всем игрокам" if mode == "all" else "активным за 24ч"
    await callback.message.edit_text(
        f"🎁 <b>Подтверди подарок</b>\n\n"
        f"{data['label']}: <b>{data['amount']}</b> {data['unit']}\n"
        f"Получателей: {cnt} ({target_label})\n\n"
        f"Итого: {data['amount'] * cnt} {data['unit']}",
        reply_markup=kb, parse_mode="HTML"
    )


@dp.message(AdminGiftStates.confirming)
async def process_ag_mid(message: Message, state: FSMContext):
    """Обработка ввода MID(ов)"""
    if not is_admin(message.from_user.id):
        return
    data = await state.get_data()
    if data.get("mode") != "mid":
        return
    mids = [x for x in message.text.split() if x.isdigit()]
    if not mids:
        await message.answer("❌ Введи хотя бы один MID"); return
    await state.update_data(mids=mids)
    await state.set_state(AdminGiftStates.entering_count)
    # Ищем пользователей
    c2 = db.conn.cursor()
    found = []
    for mid in mids:
        c2.execute("SELECT user_id, first_name FROM users WHERE custom_id=?", (int(mid),))
        row = c2.fetchone()
        if row: found.append(row)
    if not found:
        await message.answer("❌ Игроки не найдены"); return
    names = ", ".join(f"{r[1]} (MID {mids[i]})" for i, r in enumerate(found[:5]))
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"✅ Отправить ({len(found)} игр.)", callback_data="ag_send:mid")],
        [InlineKeyboardButton(text="❌ Отмена", callback_data="ag_cancel")],
    ])
    await message.answer(
        f"🎁 <b>Подтверди подарок</b>\n\n"
    ...
    await state.update_data(found_users=[r[0] for r in found])


@dp.callback_query(F.data.startswith("ag_send:"))
async def callback_ag_send(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id):
        return
    await callback.answer()
    data = await state.get_data()
    ...
    c2 = db.conn.cursor()
    if mode == "mid":
        user_ids = data.get("found_users", [])
    elif mode == "active":
        c2.execute("SELECT user_id FROM users WHERE banned=0 AND last_mining > datetime('now','-1 day')")
        user_ids = [r[0] for r in c2.fetchall()]
    else:
        c2.execute("SELECT user_id FROM users WHERE banned=0")
        user_ids = [r[0] for r in c2.fetchall()]
    ...
    for uid in user_ids:
        try:
            if resource == "ton":
                ...
                db.cursor.execute("UPDATE users SET stars_balance=COALESCE(stars_balance,0)+? WHERE user_id=?", (amount, uid))
            elif resource == "box":
                db.cursor.execute("UPDATE users SET boxes_count=COALESCE(boxes_count,0)+? WHERE user_id=?", (amount, uid))
            elif resource == "exp":
                db.cursor.execute("UPDATE users SET exp=COALESCE(exp,0)+? WHERE user_id=?", (amount, uid))
            db.conn.commit()
            try:
                await bot.send_message(uid,
                    f"🎁 <b>Подарок от администрации!</b>\n\n"
                    f"{label}: +{amount} {unit}\n\n"
                    f"<i>Спасибо, что играешь с нами!</i>",
                    parse_mode="HTML")
                sent += 1
            except Exception:
                sent += 1  # начислили, но уведомить не удалось
        except Exception:
            failed += 1

    await callback.message.edit_text(
        f"✅ <b>Подарок отправлен!</b>\n\n"
        f"Ресурс: {label} +{amount} {unit}\n"
...
@dp.callback_query(F.data == "ag_cancel")
async def callback_ag_cancel(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id):
        return
    await state.clear()
...
async def auction_finalize_scheduler():
    """Завершает истёкшие аукционы каждую минуту."""
    while True:
        try:
            c2 = db.conn.cursor()
            c2.execute("""
                SELECT id, seller_id, item_name, current_price, top_bidder
                FROM auctions WHERE is_active=1 AND ends_at <= datetime('now')
            """)
            for r in c2.fetchall():
                aid, seller_id, iname, cur_price, top_bidder = r
                db.cursor.execute("UPDATE auctions SET is_active=0 WHERE id=?", (aid,))
                db.conn.commit()
                if top_bidder:
                    db.cursor.execute("UPDATE users SET gems=gems+? WHERE user_id=?", (cur_price, seller_id))
                    db.conn.commit()
                    try:
                        await bot.send_message(top_bidder,
                            f"🎉 <b>Ты выиграл аукцион!</b>\n📦 {iname}\n💰 Ставка: {cur_price} тон",
                            parse_mode="HTML")
                        await bot.send_message(seller_id,
                            f"💰 <b>Аукцион завершён!</b>\n📦 {iname}\n+{cur_price} тон зачислено",
                            parse_mode="HTML")
                    except Exception: pass
                else:
                    try:
                        await bot.send_message(seller_id,
                            f"ℹ️ Аукцион «{iname}» завершён без ставок.", parse_mode="HTML")
                    except Exception: pass
        except Exception as e:
            logger.error(f"auction_finalize_scheduler: {e}")
        await asyncio.sleep(60)


@dp.callback_query(F.data == "auction_list")
async def callback_auction_list(callback: CallbackQuery):
    await callback.answer()
    c2 = db.conn.cursor()
    c2.execute("""
        SELECT id, item_type, item_name, hash_rate, current_price, buyout_price, top_bidder, ends_at
        FROM auctions WHERE is_active=1 AND ends_at > datetime('now')
    ...
    buttons = []
    if not rows:
        text += "Активных лотов нет.\nВыстави своё оборудование!"
    for r in rows:
        aid, itype, iname, hr, cur, buyout, top, ends = r
        ends_dt = datetime.fromisoformat(ends)
        mins = max(0, int((ends_dt - datetime.now()).total_seconds() // 60))
        h, m = divmod(mins, 60)
        ts = f"{h}ч {m}м" if h else f"{m}м"
        bp_str = f" | 🛍{buyout}т" if buyout else ""
        ico = "⚡" if itype == "gpu" else "❄️"
        buttons.append([InlineKeyboardButton(
            text=f"{ico} {iname} — {cur}т{bp_str} ⏰{ts}",
            callback_data=f"auc_view:{aid}"
        )])
    buttons.append([InlineKeyboardButton(text="➕ Выставить лот",  callback_data="auc_sell_choose")])
    buttons.append([InlineKeyboardButton(text="📋 Мои лоты",       callback_data="auc_my")])
...
@dp.callback_query(F.data.startswith("auc_view:"))
async def callback_auc_view(callback: CallbackQuery):
    await callback.answer()
    aid = int(callback.data.split(":")[1])
    c2 = db.conn.cursor()
    c2.execute("SELECT id,seller_id,item_type,item_name,hash_rate,wear,start_price,current_price,buyout_price,top_bidder,ends_at,is_active FROM auctions WHERE id=?", (aid,))
    r = c2.fetchone()
    if not r or not r[11]:
        await callback.answer("Лот не найден или завершён", show_alert=True); return
    aid, sid, itype, iname, hr, wear, sp, cur, buyout, top_bidder, ends, _ = r
    ends_dt = datetime.fromisoformat(ends)
    mins = max(0, int((ends_dt - datetime.now()).total_seconds() // 60))
    h, m2 = divmod(mins, 60)
    ts = f"{h}ч {m2}м" if h else f"{m2}м"
    desc = f"⚡ {hr:.0f} MH/s | 🛡 {wear}%" if itype == "gpu" else f"❄️ кулер | 🛡 {wear}%"
    top_name = "нет"
    if top_bidder:
        u = db.get_user(top_bidder)
        top_name = u[3] if u else str(top_bidder)
    bp_str = f"\n🛍 Купить сейчас: {buyout} тон" if buyout else ""
    text = (
        f"🔨 <b>{iname}</b>\n\n"
        f"<blockquote>{desc}\n"
        f"Старт: {sp} тон\n"
        f"Ставка: {cur} тон | Лидер: {top_name}{bp_str}\n"
        f"⏰ Осталось: {ts}</blockquote>"
    )
    min_bid = cur + max(1, cur // 20)
    buttons = []
    uid = callback.from_user.id
    if uid != sid:
        buttons.append([InlineKeyboardButton(text=f"⬆️ Ставка {min_bid} тон", callback_data=f"auc_bid:{aid}:{min_bid}")])
        if buyout:
            buttons.append([InlineKeyboardButton(text=f"🛍 Купить сейчас {buyout} тон", callback_data=f"auc_buyout:{aid}")])
        buttons.append([InlineKeyboardButton(text="✏️ Своя сумма", callback_data=f"auc_bid_custom:{aid}")])
    else:
        buttons.append([InlineKeyboardButton(text="❌ Снять лот", callback_data=f"auc_cancel:{aid}")])
    buttons.append([InlineKeyboardButton(text="« Назад", callback_data="auction_list")])
    await callback.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons), parse_mode="HTML")


@dp.callback_query(F.data.startswith("auc_bid:"))
async def callback_auc_bid(callback: CallbackQuery):
    await callback.answer()
    parts = callback.data.split(":")
    aid, amount = int(parts[1]), int(parts[2])
    await _do_auc_bid(callback, aid, amount)


@dp.callback_query(F.data.startswith("auc_bid_custom:"))
async def callback_auc_bid_custom(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    aid = int(callback.data.split(":")[1])
    await state.set_state(AuctionStates.waiting_bid)
    await state.update_data(auction_id=aid)
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="❌ Отмена", callback_data=f"auc_view:{aid}")]
    ])
    await callback.message.edit_text("✏️ Введи сумму ставки (тон):", reply_markup=kb)


@dp.message(AuctionStates.waiting_bid)
async def process_auc_bid_input(message: Message, state: FSMContext):
    if not message.text or not message.text.isdigit():
        await message.answer("❌ Введи целое число"); return
    data = await state.get_data()
    aid = data.get("auction_id")
    await state.clear()
    await _do_auc_bid(message, aid, int(message.text))


async def _do_auc_bid(target, aid: int, amount: int):
    uid = target.from_user.id
    c2 = db.conn.cursor()
    c2.execute("SELECT id,seller_id,item_name,current_price,top_bidder,ends_at,is_active FROM auctions WHERE id=?", (aid,))
    r = c2.fetchone()
    if not r or not r[6]:
        txt = "❌ Лот не найден или завершён"
    elif r[1] == uid:
        txt = "❌ Нельзя ставить на свой лот"
    elif datetime.fromisoformat(r[5]) <= datetime.now():
        txt = "❌ Аукцион уже завершён"
    elif amount <= r[3]:
        txt = f"❌ Ставка должна быть больше {r[3]} тон"
    else:
        user = db.get_user(uid)
        if int(user[4]) < amount:
            txt = f"❌ Недостаточно тон (нужно {amount}, есть {fmt(user[4])})"
        else:
            prev_bidder, prev_amount = r[4], r[3]
            if prev_bidder and prev_bidder != uid:
                db.cursor.execute("UPDATE users SET gems=gems+? WHERE user_id=?", (prev_amount, prev_bidder))
                try:
                    await bot.send_message(prev_bidder,
                        f"⚠️ <b>Вас перебили на аукционе!</b>\n\n"
                        f"🔨 {r[2]}\n"
                        f"Ваша ставка {prev_amount} тон возвращена.\n"
                        f"Новая ставка: {fmt(amount)} тон",
                        parse_mode="HTML")
                except Exception: pass
            db.cursor.execute("UPDATE users SET gems=gems-? WHERE user_id=?", (amount, uid))
            db.cursor.execute("UPDATE auctions SET current_price=?, top_bidder=? WHERE id=?", (amount, uid, aid))
            db.conn.commit()
            txt = f"✅ Ставка {fmt(amount)} тон принята!\n🔨 {r[2]}"
    kb = _auction_back_kb()
    if isinstance(target, Message):
...
@dp.callback_query(F.data.startswith("auc_buyout:"))
async def callback_auc_buyout(callback: CallbackQuery):
    await callback.answer()
    aid = int(callback.data.split(":")[1])
    c2 = db.conn.cursor()
    c2.execute("SELECT id,seller_id,item_name,current_price,buyout_price,top_bidder,is_active FROM auctions WHERE id=?", (aid,))
    r = c2.fetchone()
    if not r or not r[6] or not r[4]:
        await callback.answer("Недоступно", show_alert=True); return
    uid = callback.from_user.id
    buyout = r[4]
    user = db.get_user(uid)
    if int(user[4]) < buyout:
        await callback.answer(f"❌ Нужно {buyout} тон, есть {fmt(user[4])}", show_alert=True); return
    prev_bidder, prev_amount = r[5], r[3]
    if prev_bidder and prev_bidder != uid:
        db.cursor.execute("UPDATE users SET gems=gems+? WHERE user_id=?", (prev_amount, prev_bidder))
        try:
            await bot.send_message(prev_bidder, f"ℹ️ Лот «{r[2]}» выкуплен мгновенно. Ставка {prev_amount} тон возвращена.")
        except Exception: pass
    db.cursor.execute("UPDATE users SET gems=gems-? WHERE user_id=?", (buyout, uid))
    db.cursor.execute("UPDATE users SET gems=gems+? WHERE user_id=?", (buyout, r[1]))
    db.cursor.execute("UPDATE auctions SET is_active=0, top_bidder=? WHERE id=?", (uid, aid))
    db.conn.commit()
    try:
        await bot.send_message(r[1], f"🎉 <b>Лот выкуплен!</b>\n📦 {r[2]}\n💰 +{buyout} тон", parse_mode="HTML")
    except Exception: pass
    await callback.message.edit_text(
        f"✅ <b>Куплено!</b>\n📦 {r[2]}\n💰 {buyout} тон",
        reply_markup=_auction_back_kb(), parse_mode="HTML"
    )


@dp.callback_query(F.data.startswith("auc_cancel:"))
async def callback_auc_cancel(callback: CallbackQuery):
    await callback.answer()
    aid = int(callback.data.split(":")[1])
    c2 = db.conn.cursor()
    c2.execute("SELECT seller_id,item_name,current_price,top_bidder,is_active FROM auctions WHERE id=?", (aid,))
    r = c2.fetchone()
    if not r or not r[4] or r[0] != callback.from_user.id:
        await callback.answer("Нельзя снять", show_alert=True); return
    if r[3]:
        db.cursor.execute("UPDATE users SET gems=gems+? WHERE user_id=?", (r[2], r[3]))
        try:
            await bot.send_message(r[3], f"ℹ️ Лот «{r[1]}» снят продавцом. Ставка {r[2]} тон возвращена.")
        except Exception: pass
    db.cursor.execute("UPDATE auctions SET is_active=0 WHERE id=?", (aid,))
    db.conn.commit()
    await callback.answer("✅ Лот снят", show_alert=True)
    await callback_auction_list(callback)


@dp.callback_query(F.data == "auc_my")
async def callback_auc_my(callback: CallbackQuery):
    await callback.answer()
    c2 = db.conn.cursor()
    c2.execute("SELECT id,item_name,current_price,ends_at FROM auctions WHERE seller_id=? AND is_active=1", (callback.from_user.id,))
    rows = c2.fetchall()
    text = "📋 <b>Мои лоты на аукционе</b>\n\n"
    buttons = []
    if not rows:
        text += "У тебя нет активных аукционных лотов."
    for r in rows:
        aid, iname, cur, ends = r
        ends_dt = datetime.fromisoformat(ends)
        mins = max(0, int((ends_dt - datetime.now()).total_seconds() // 60))
        h, m2 = divmod(mins, 60)
        text += f"• {iname} — {cur} тон | ⏰{h}ч{m2}м\n"
        buttons.append([InlineKeyboardButton(text=f"❌ Снять: {iname}", callback_data=f"auc_cancel:{aid}")])
    buttons.append([InlineKeyboardButton(text="« Аукцион", callback_data="auction_list")])
    await callback.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons), parse_mode="HTML")


@dp.callback_query(F.data == "auc_sell_choose")
async def callback_auc_sell_choose(callback: CallbackQuery):
    await callback.answer()
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⚡ GPU",    callback_data="auc_inv_gpu")],
        [InlineKeyboardButton(text="❄️ Кулер", callback_data="auc_inv_cooler")],
        [InlineKeyboardButton(text="« Назад",  callback_data="auction_list")],
    ])
    await callback.message.edit_text("🔨 <b>Выставить на аукцион</b>\n\nВыбери тип предмета:", reply_markup=kb, parse_mode="HTML")


@dp.callback_query(F.data == "auc_inv_gpu")
async def callback_auc_inv_gpu(callback: CallbackQuery):
    await callback.answer()
    gpus = db.get_free_gpus(callback.from_user.id)
    buttons = []
    for g in gpus:
        buttons.append([InlineKeyboardButton(
            text=f"⚡ {g[2]} | {float(g[3] or 0):.0f} MH/s | 🛡{g[5] or 100}%",
            callback_data=f"auc_set_gpu:{g[0]}"
        )])
    buttons.append([InlineKeyboardButton(text="« Назад", callback_data="auc_sell_choose")])
    await callback.message.edit_text(
        "⚡ Выбери GPU:" if buttons[:-1] else "❌ Нет свободных GPU",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons), parse_mode="HTML"
    )


@dp.callback_query(F.data == "auc_inv_cooler")
async def callback_auc_inv_cooler(callback: CallbackQuery):
    await callback.answer()
    coolers = db.get_free_coolers(callback.from_user.id)
    buttons = []
    for co in coolers:
        buttons.append([InlineKeyboardButton(
            text=f"❄️ {co[2]} | охл.{co[4] or 0}",
            callback_data=f"auc_set_cooler:{co[0]}"
        )])
    buttons.append([InlineKeyboardButton(text="« Назад", callback_data="auc_sell_choose")])
    await callback.message.edit_text(
        "❄️ Выбери кулер:" if buttons[:-1] else "❌ Нет свободных кулеров",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons), parse_mode="HTML"
    )


@dp.callback_query(F.data.startswith("auc_set_gpu:"))
async def callback_auc_set_gpu(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    gid = int(callback.data.split(":")[1])
    await state.set_state(AuctionStates.waiting_start_price)
    await state.update_data(item_type="gpu", item_id=gid)
    await callback.message.edit_text(
        "💰 Введи стартовую цену (тон).\n"
        "<i>Чтобы добавить цену мгновенной покупки — напиши два числа через пробел:\n"
        "Пример: 500 2000</i>",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="❌ Отмена", callback_data="auction_list")]
        ]),
        parse_mode="HTML"
    )


@dp.callback_query(F.data.startswith("auc_set_cooler:"))
async def callback_auc_set_cooler(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    cid = int(callback.data.split(":")[1])
    await state.set_state(AuctionStates.waiting_start_price)
    await state.update_data(item_type="cooler", item_id=cid)
    await callback.message.edit_text(
        "💰 Введи стартовую цену (тон).\n"
        "<i>Необязательно: добавь цену мгновенной покупки через пробел\nПример: 200 800</i>",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="❌ Отмена", callback_data="auction_list")]
        ]),
        parse_mode="HTML"
...
@dp.message(AuctionStates.waiting_start_price)
async def process_auc_start_price(message: Message, state: FSMContext):
    ...
    start_price = int(parts[0])
    buyout = int(parts[1]) if len(parts) > 1 and parts[1].isdigit() else None
    data = await state.get_data()
    await state.clear()
    itype, iid = data["item_type"], data["item_id"]
    uid = message.from_user.id
    c2 = db.conn.cursor()
    if itype == "gpu":
        c2.execute("SELECT name, hash_rate, wear FROM user_gpus WHERE id=? AND user_id=?", (iid, uid))
        row = c2.fetchone()
        if not row: await message.answer("❌ GPU не найден"); return
        iname, hr_val, wear = row[0], float(row[1] or 0), row[2] or 100
        cool_val = 0
    else:
        c2.execute("SELECT name, cooling_power, wear FROM user_coolers WHERE id=? AND user_id=?", (iid, uid))
        row = c2.fetchone()
        if not row: await message.answer("❌ Кулер не найден"); return
        iname, cool_val, wear = row[0], row[1] or 0, row[2] if len(row) > 2 else 100
        hr_val = 0
    ends_at = (datetime.now() + timedelta(hours=24)).isoformat()
    db.cursor.execute("""
        INSERT INTO auctions (seller_id,item_type,item_id,item_name,hash_rate,wear,
                              start_price,current_price,buyout_price,ends_at,is_active)
        VALUES (?,?,?,?,?,?,?,?,?,?,1)
    """, (uid, itype, iid, iname, hr_val, wear, start_price, start_price, buyout, ends_at))
    db.conn.commit()
    bp_str = f"\n🛍 Мгновенная покупка: {buyout} тон" if buyout else ""
    await message.answer(
        f"✅ <b>Лот выставлен на аукцион!</b>\n\n📦 {iname}\n💰 Старт: {start_price} тон{bp_str}\n⏰ 24 часа",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔨 Аукцион", callback_data="auction_list")]
        ]),
        parse_mode="HTML"
...
TOURNAMENT_PRIZES = {1: 0.50, 2: 0.30, 3: 0.20}
...
async def tournament_scheduler():
    while True:
        try:
            c2 = db.conn.cursor()
            c2.execute("SELECT id,end_at,prize_pool FROM tournaments WHERE status='active' LIMIT 1")
            active = c2.fetchone()
            if active:
                tid, end_at, prize_pool = active
                if datetime.fromisoformat(end_at) <= datetime.now():
                    c2.execute("SELECT user_id,mined FROM tournament_scores WHERE tournament_id=? ORDER BY mined DESC LIMIT 3", (tid,))
                    top3 = c2.fetchall()
                    db.cursor.execute("UPDATE tournaments SET status='finished' WHERE id=?", (tid,))
                    db.conn.commit()
                    medals = {1: "🥇", 2: "🥈", 3: "🥉"}
                    for place, (uid, mined) in enumerate(top3, 1):
                        prize = int(prize_pool * TOURNAMENT_PRIZES.get(place, 0))
                        if prize:
                            db.cursor.execute("UPDATE users SET gems=gems+? WHERE user_id=?", (prize, uid))
                            db.conn.commit()
                            try:
                                await bot.send_message(uid,
                                    f"{medals[place]} <b>Турнир завершён!</b>\n"
                                    f"Место: {place}\n💰 Награда: +{fmt(prize)} тон", parse_mode="HTML")
                            except Exception: pass
                    start = datetime.now()
                    db.cursor.execute(
                        "INSERT INTO tournaments (start_at,end_at,prize_pool,status) VALUES (?,?,?,?)",
                        (start.isoformat(), (start + timedelta(days=7)).isoformat(), 10000, 'active'))
                    db.conn.commit()
            else:
                start = datetime.now()
                db.cursor.execute(
            ...
            logger.error(f"tournament_scheduler: {e}")
        await asyncio.sleep(3600)


def _add_tournament_score(user_id: int, mined: int):
    try:
        c2 = db.conn.cursor()
        c2.execute("SELECT id FROM tournaments WHERE status='active' LIMIT 1")
        t = c2.fetchone()
        if not t: return
        db.cursor.execute("""
            INSERT INTO tournament_scores (tournament_id,user_id,mined)
            VALUES (?,?,?)
            ON CONFLICT(tournament_id,user_id) DO UPDATE SET mined=mined+excluded.mined
        """, (t[0], user_id, mined))
        db.conn.commit()
    except Exception: pass
...
@dp.callback_query(F.data == "tournament_menu")
async def show_tournament(target):
    uid = target.from_user.id
    c2 = db.conn.cursor()
    c2.execute("SELECT id,start_at,end_at,prize_pool FROM tournaments WHERE status='active' LIMIT 1")
    t = c2.fetchone()
    if not t:
        text = "🏆 Турнир ещё не начался."
        kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="« Назад", callback_data="back_to_menu")]])
    else:
        tid, start, end, pool = t
        days_left = max(0, (datetime.fromisoformat(end) - datetime.now()).days)
        c2.execute("""
            SELECT u.first_name, ts.mined FROM tournament_scores ts
            JOIN users u ON u.user_id=ts.user_id
            WHERE ts.tournament_id=? ORDER BY ts.mined DESC LIMIT 10
        """, (tid,))
        top = c2.fetchall()
        c2.execute("SELECT mined FROM tournament_scores WHERE tournament_id=? AND user_id=?", (tid, uid))
        my = c2.fetchone()
        c2.execute("SELECT 1 FROM tournament_scores WHERE tournament_id=? AND user_id=?", (tid, uid))
        is_registered = c2.fetchone() is not None
        medals = ["🥇","🥈","🥉"]
        text = (f"🏆 <b>Еженедельный турнир</b>\n\n"
                f"<blockquote>💰 Призовой фонд: {pool} тон\n"
                f"⏰ Осталось: {days_left} дн.\n"
                f"🥇 50% | 🥈 30% | 🥉 20%</blockquote>\n\n")
        for i, (name, mined) in enumerate(top):
            m = medals[i] if i < 3 else f"{i+1}."
            text += f"{m} {name} — {mined} тон\n"
        if my:
            text += f"\n📊 Твои тоны в турнире: {my[0]}"
        join_btn = (
            InlineKeyboardButton(text="✅ Ты участвуешь", callback_data="tournament_already")
            if is_registered else
            InlineKeyboardButton(text="🏆 Участвовать", callback_data=f"tournament_join_{tid}")
        )
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [join_btn],
            [InlineKeyboardButton(text="« Назад", callback_data="back_to_menu")]
...
@dp.callback_query(F.data == "tournament_already")
async def callback_tournament_already(callback: CallbackQuery):
    await callback.answer("✅ Ты уже участвуешь в турнире!", show_alert=True)


@dp.callback_query(F.data.startswith("tournament_join_"))
async def callback_tournament_join(callback: CallbackQuery):
    await callback.answer()
    uid = callback.from_user.id
    ...
    if c2.fetchone():
        ...
        return
    try:
        c2.execute("INSERT INTO tournament_scores (tournament_id, user_id, mined) VALUES (?,?,0)", (tid, uid))
        db.conn.commit()
        await callback.answer("🏆 Ты записан в турнир!", show_alert=True)
        await show_tournament(callback)
    except Exception as e:
...
WORLD_EVENTS_LIST = [
    {"key":"double_ton",    "title":"⚡ Двойной доход!",     "desc":"×2 тон за майнинг 30 минут!",         "effect":"ton_mult",   "magnitude":2.0, "dur":30},
    {"key":"double_box",    "title":"📦 Бокс-бонус!",        "desc":"Шанс боксов ×3 на 30 минут!",         "effect":"box_mult",   "magnitude":3.0, "dur":30},
    {"key":"exp_boost",     "title":"✨ Опыт ×2!",            "desc":"Двойной опыт за майнинг 1 час!",      "effect":"exp_mult",   "magnitude":2.0, "dur":60},
    {"key":"cheap_repair",  "title":"🔧 Скидка на ремонт!",  "desc":"Ремонт GPU бесплатен 1 час!",         "effect":"free_repair","magnitude":1.0, "dur":60},
    {"key":"ton_rain",      "title":"🌧 Дождь из тонов!",    "desc":"Всем активным игрокам +300 тон!",     "effect":"instant_ton","magnitude":300, "dur":1},
]
...
def get_active_event() -> dict | None:
    try:
        c2 = db.conn.cursor()
        c2.execute("SELECT event_key,title,effect,magnitude FROM world_events WHERE is_active=1 AND ends_at > datetime('now') LIMIT 1")
        r = c2.fetchone()
        if not r: return None
        return {"key": r[0], "title": r[1], "effect": r[2], "magnitude": r[3]}
    except Exception:
        return None


async def world_event_scheduler():
    while True:
        await asyncio.sleep(random.randint(3600, 7200))  # каждые 1-2 часа
        try:
            db.cursor.execute("UPDATE world_events SET is_active=0 WHERE ends_at <= datetime('now')")
            db.conn.commit()
            ev = random.choice(WORLD_EVENTS_LIST)
            started = datetime.now()
            ends = started + timedelta(minutes=ev["dur"])
            db.cursor.execute("""
                INSERT INTO world_events (event_key,title,description,effect,magnitude,started_at,ends_at,is_active)
                VALUES (?,?,?,?,?,?,?,1)
            """, (ev["key"], ev["title"], ev["desc"], ev["effect"], ev["magnitude"],
                  started.isoformat(), ends.isoformat()))
            db.conn.commit()
            event_text = f"🌍 <b>Мировое событие!</b>\n\n{ev['title']}\n{ev['desc']}\n\n⏰ {ev['dur']} минут"
            try:
                await bot.send_message(config.CHANNEL_USERNAME, event_text, parse_mode="HTML")
                print(f"✅ Мировое событие отправлено в {config.CHANNEL_USERNAME}")
            except Exception as ch_e:
                print(f"⚠️ Не удалось отправить событие в канал {config.CHANNEL_USERNAME}: {ch_e}")
            if ev["effect"] == "instant_ton":
                db.cursor.execute("UPDATE users SET gems=gems+? WHERE user_id>0 AND banned=0", (int(ev["magnitude"]),))
                db.conn.commit()
        except Exception as e:
            logger.error(f"world_event_scheduler: {e}")
...
async def cmd_duel(message: Message):
    ...
    if len(args) < 3 or not args[1].isdigit() or not args[2].isdigit():
        await message.answer(
            "⚔️ <b>Дуэль</b>\n\n"
            "Использование: <code>/duel MID СТАВКА</code>\n\n"
            "Победитель определяется по хешрейту + удача (±20%).\n"
            "Победитель забирает обе ставки.",
            parse_mode="HTML"); return
    opp_mid, bet = int(args[1]), int(args[2])
    if bet <= 0:
        await message.answer("❌ Ставка должна быть больше 0"); return
    c2 = db.conn.cursor()
    c2.execute("SELECT user_id,first_name FROM users WHERE custom_id=?", (opp_mid,))
    opp = c2.fetchone()
    if not opp:
        await message.answer("❌ Игрок не найден"); return
    opp_id, opp_name = opp
    if opp_id == message.from_user.id:
        await message.answer("❌ Нельзя вызвать себя на дуэль"); return
    me = db.get_user(message.from_user.id)
    if int(me[4]) < bet:
        await message.answer(f"❌ Недостаточно тон (нужно {fmt(bet)}, есть {fmt(me[4])})"); return
    db.cursor.execute(
        "INSERT INTO duels (challenger_id,opponent_id,bet,status) VALUES (?,?,?,'pending')",
        (message.from_user.id, opp_id, bet))
    duel_id = db.cursor.lastrowid
    db.conn.commit()
    kb = InlineKeyboardMarkup(inline_keyboard=[
    ...
    try:
        await bot.send_message(opp_id,
            f"⚔️ <b>{me[3]} вызывает тебя на дуэль!</b>\n\n💰 Ставка: {fmt(bet)} тон\nПобедитель забирает всё!",
            reply_markup=kb, parse_mode="HTML")
        await message.answer(f"⚔️ Вызов отправлен игроку {opp_name}! Ожидай ответа.", parse_mode="HTML")
    except Exception:
...
async def callback_duel_accept(callback: CallbackQuery):
    ...
    c2 = db.conn.cursor()
    c2.execute("SELECT * FROM duels WHERE id=? AND status='pending'", (duel_id,))
    d = c2.fetchone()
    if not d:
        await callback.answer("Дуэль уже завершена или отменена", show_alert=True)
        return
    did, chal_id, opp_id, bet, *_ = d
    if callback.from_user.id != opp_id:
        await callback.answer("Это не твой вызов", show_alert=True)
        return

    chal = db.get_user(chal_id)
    opp  = db.get_user(opp_id)

    # Проверяем баланс обоих ДО списания
    if int(chal[4]) < bet:
        await callback.answer(
            f"❌ У вызывающего ({chal[3]}) не хватает тон (нужно {fmt(bet)}, есть {fmt(chal[4])})",
            show_alert=True
        )
        return
    if int(opp[4]) < bet:
        await callback.answer(
            f"❌ У тебя не хватает тон (нужно {fmt(bet)}, есть {fmt(opp[4])})",
            show_alert=True
        )
        return

    # Сразу помечаем дуэль как в процессе, чтоб не приняли дважды
    db.cursor.execute("UPDATE duels SET status='in_progress' WHERE id=?", (duel_id,))
    db.conn.commit()

    # Списываем ставки
    db.cursor.execute("UPDATE users SET gems=gems-? WHERE user_id=?", (bet, chal_id))
    db.cursor.execute("UPDATE users SET gems=gems-? WHERE user_id=?", (bet, opp_id))
    db.conn.commit()

    # Обновляем кнопку → «⏳ Дуэль в процессе...»
    try:
        await callback.message.edit_reply_markup(reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="⏳ Дуэль в процессе...", callback_data="noop")]
        ]))
    except Exception: pass

    # Кидаем кубики принявшему (opp) и создавшему (chal) одновременно
    # Каждому кидаем ОДИН кубик — его значение и есть результат
    try:
        d1_opp = await bot.send_dice(opp_id, emoji="🎲")
    except Exception:
        d1_opp = None
    try:
        d1_chal = await bot.send_dice(chal_id, emoji="🎲")
    except Exception:
        d1_chal = None

    await asyncio.sleep(3)

    # Результат: каждый кубик независим, у каждого игрока свой броcок
    chal_roll = d1_chal.dice.value if d1_chal else random.randint(1, 6)
    opp_roll  = d1_opp.dice.value  if d1_opp  else random.randint(1, 6)
    # Второй кубик каждого не используется для подсчёта — только анимация

    # Ничья — перебрасываем программно
    while chal_roll == opp_roll:
        chal_roll = random.randint(1, 6)
        opp_roll  = random.randint(1, 6)

    winner_id = chal_id if chal_roll > opp_roll else opp_id
    loser_id  = opp_id  if winner_id == chal_id else chal_id
    w_roll    = chal_roll if winner_id == chal_id else opp_roll
    l_roll    = opp_roll  if winner_id == chal_id else chal_roll

    commission = max(1, int(bet * 2 * 0.05))
    prize = bet * 2 - commission
    db.cursor.execute("UPDATE users SET gems=gems+? WHERE user_id=?", (prize, winner_id))
    db.cursor.execute("UPDATE bot_balance SET balance=balance+? WHERE id=1", (commission,))
    db.cursor.execute("UPDATE duels SET status='finished',winner_id=? WHERE id=?", (winner_id, duel_id))
    db.conn.commit()
    ...
    result_text = (
        f"⚔️ <b>Дуэль завершена!</b>\n\n"
        f"<blockquote>"
        f"🏆 {escape_html(w[3])}: 🎲 {w_roll}\n"
        f"💀 {escape_html(l[3])}: 🎲 {l_roll}"
        f"</blockquote>\n\n"
        f"Победитель: <b>{escape_html(w[3])}</b>\n"
        f"💰 Приз: {fmt(prize)} тон"
    )

    # Обновляем сообщение с вызовом у принявшего
    try:
        await callback.message.edit_text(result_text, parse_mode="HTML")
    except Exception: pass

    # Результат создавшему
    try:
        await bot.send_message(chal_id, result_text, parse_mode="HTML")
    except Exception: pass

    # Результат принявшему (если редактирование не сработало)
    try:
        await bot.send_message(opp_id, result_text, parse_mode="HTML")
    except Exception: pass
...
async def callback_duel_decline(callback: CallbackQuery):
    ...
    duel_id = int(callback.data.split(":")[1])
    c2 = db.conn.cursor()
    c2.execute("SELECT challenger_id FROM duels WHERE id=?", (duel_id,))
    r = c2.fetchone()
    db.cursor.execute("UPDATE duels SET status='declined' WHERE id=?", (duel_id,))
    db.conn.commit()
    if r:
        try:
            await bot.send_message(r[0], "⚔️ Твой вызов на дуэль был отклонён.")
        except Exception: pass
    try:
        await callback.message.edit_text(
            "❌ <b>Дуэль отклонена</b>",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="❌ Вызов отклонён", callback_data="noop")]
            ]),
            parse_mode="HTML"
        )
    except Exception: pass
...
async def cmd_gift(message: Message):
    ...
    if len(args) < 3:
        await message.answer(
            "🎁 <b>Подарок</b>\n\n"
            "<code>/gift MID box</code> — подарить бокс\n"
            "<code>/gift MID gpu</code> — подарить GPU\n"
            "<code>/gift MID cooler</code> — подарить кулер",
            parse_mode="HTML"); return
    if not args[1].isdigit():
        await message.answer("❌ Укажи MID получателя"); return
    mid = int(args[1])
    c2 = db.conn.cursor()
    c2.execute("SELECT user_id,first_name FROM users WHERE custom_id=?", (mid,))
    rec = c2.fetchone()
    if not rec:
        await message.answer("❌ Получатель не найден"); return
    rec_id, rec_name = rec
    if rec_id == message.from_user.id:
        await message.answer("❌ Нельзя дарить себе"); return
    gift_type = args[2].lower()
    me = db.get_user(message.from_user.id)
    if gift_type == "box":
        boxes = me[16] if len(me) > 16 and me[16] else 0
        if boxes < 1:
            await message.answer("❌ У тебя нет боксов"); return
        db.cursor.execute("UPDATE users SET boxes_count=boxes_count-1 WHERE user_id=?", (message.from_user.id,))
        db.cursor.execute("UPDATE users SET boxes_count=boxes_count+1 WHERE user_id=?", (rec_id,))
        db.conn.commit()
        try:
            await bot.send_message(rec_id,
                f"🎁 <b>Тебе подарили бокс!</b>\nОт: <b>{me[3]}</b>",
                parse_mode="HTML")
        except Exception: pass
        await message.answer(f"✅ Бокс подарен игроку {rec_name}!", parse_mode="HTML")
    elif gift_type == "gpu":
        gpus = db.get_free_gpus(message.from_user.id)
        if not gpus:
            await message.answer("❌ Нет свободных GPU"); return
        buttons = [[InlineKeyboardButton(
            text=f"⚡ {g[2]} | {float(g[3] or 0):.0f} MH/s | 🛡{g[5] or 100}%",
            callback_data=f"gift_gpu:{rec_id}:{g[0]}"
        )] for g in gpus[:10]]
        await message.answer(
            f"🎁 Выбери GPU для подарка {rec_name}:",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons),
            parse_mode="HTML")
    elif gift_type == "cooler":
        coolers = db.get_free_coolers(message.from_user.id)
        if not coolers:
            await message.answer("❌ Нет свободных кулеров"); return
        buttons = [[InlineKeyboardButton(
            text=f"❄️ {co[2]} | охл.{co[4] or 0}",
            callback_data=f"gift_cooler:{rec_id}:{co[0]}"
        )] for co in coolers[:10]]
        await message.answer(
            f"🎁 Выбери кулер для подарка {rec_name}:",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons),
            parse_mode="HTML")
    else:
        await message.answer("❌ Тип: box, gpu, cooler")


@dp.callback_query(F.data.startswith("gift_gpu:"))
async def callback_gift_gpu(callback: CallbackQuery):
    await callback.answer()
    _, rec_id, gid = callback.data.split(":")
    rec_id, gid = int(rec_id), int(gid)
    c2 = db.conn.cursor()
    c2.execute("SELECT name FROM user_gpus WHERE id=? AND user_id=?", (gid, callback.from_user.id))
    g = c2.fetchone()
    if not g:
        await callback.answer("❌ GPU не найден", show_alert=True); return
    db.cursor.execute("UPDATE user_gpus SET user_id=?,rig_id=NULL,slot_index=NULL,is_installed=0 WHERE id=?", (rec_id, gid))
    db.conn.commit()
    db.recalculate_user_hashrate(callback.from_user.id)
    db.recalculate_user_hashrate(rec_id)
    me = db.get_user(callback.from_user.id)
    rec = db.get_user(rec_id)
    try:
        await bot.send_message(rec_id,
            f"🎁 <b>Тебе подарили GPU!</b>\n⚡ {g[0]}\nОт: <b>{me[3]}</b>",
            parse_mode="HTML")
    except Exception: pass
    await callback.message.edit_text(f"✅ GPU «{g[0]}» подарен {rec[3]}!", parse_mode="HTML")


@dp.callback_query(F.data.startswith("gift_cooler:"))
async def callback_gift_cooler(callback: CallbackQuery):
    await callback.answer()
    _, rec_id, cid = callback.data.split(":")
    rec_id, cid = int(rec_id), int(cid)
    c2 = db.conn.cursor()
    c2.execute("SELECT name FROM user_coolers WHERE id=? AND user_id=?", (cid, callback.from_user.id))
    co = c2.fetchone()
    if not co:
        await callback.answer("❌ Кулер не найден", show_alert=True); return
    db.cursor.execute("UPDATE user_coolers SET user_id=?,rig_id=NULL,is_installed=0 WHERE id=?", (rec_id, cid))
    db.conn.commit()
    me = db.get_user(callback.from_user.id)
    rec = db.get_user(rec_id)
    try:
        await bot.send_message(rec_id,
            f"🎁 <b>Тебе подарили кулер!</b>\n❄️ {co[0]}\nОт: <b>{me[3]}</b>",
            parse_mode="HTML")
    except Exception: pass
    await callback.message.edit_text(f"✅ Кулер «{co[0]}» подарен {rec[3]}!", parse_mode="HTML")
...
OC_SUCCESS_CHANCE  = 0.60  # 60% успех
OC_BONUS           = 0.20  # +20% хешрейт
OC_DESTROY_CHANCE  = 0.50  # при неудаче: 50% уничтожение, 50% -10% прочности


@dp.message(Command("overclock"))
@dp.callback_query(F.data == "overclock_menu")


@dp.callback_query(F.data == "duel_help")
async def callback_duel_help(callback: CallbackQuery):
    await callback.answer()
    await callback.message.answer(
        "⚔️ <b>Дуэль</b>\n\n"
        "Вызов через команду:\n"
        "<code>/duel MID СТАВКА</code>\n\n"
        "Победитель определяется по хешрейту + удача.\n"
        "Победитель забирает обе ставки.",
        parse_mode="HTML"
    )

async def show_overclock_menu(target):
    uid = target.from_user.id
    if isinstance(target, CallbackQuery):
        await target.answer()
    gpus = db.get_free_gpus(uid)
    if not gpus:
        text = ("⚡ <b>Разгон GPU</b>\n\n"
                "Нет свободных GPU для разгона.\n\n"
                "<i>+20% хешрейт при успехе (60%)\n"
                "Уничтожение или -10% прочности при провале</i>")
        kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="« Назад", callback_data="back_to_menu")]])
    else:
        text = ("⚡ <b>Разгон GPU</b>\n\n"
                "Выбери карту для разгона:\n\n"
                "<blockquote>✅ Успех (60%): +20% хешрейт\n"
                "❌ Провал (40%): уничтожение или -10% прочности</blockquote>")
        buttons = [[InlineKeyboardButton(
            text=f"⚡ {g[2]} | {float(g[3] or 0):.0f} MH/s | 🛡{g[5] or 100}%",
            callback_data=f"oc_pick:{g[0]}"
        )] for g in gpus[:10]]
        buttons.append([InlineKeyboardButton(text="« Назад", callback_data="back_to_menu")])
        kb = InlineKeyboardMarkup(inline_keyboard=buttons)
    if isinstance(target, Message):
...
@dp.callback_query(F.data.startswith("oc_pick:"))
async def callback_oc_pick(callback: CallbackQuery):
    await callback.answer()
    gid = int(callback.data.split(":")[1])
    c2 = db.conn.cursor()
    c2.execute("SELECT name,hash_rate,wear FROM user_gpus WHERE id=? AND user_id=?", (gid, callback.from_user.id))
    g = c2.fetchone()
    if not g:
        await callback.answer("GPU не найден", show_alert=True); return
    name, hr, wear = g[0], float(g[1] or 0), g[2] or 100
    text = (f"⚡ <b>Разгон: {name}</b>\n\n"
            f"<blockquote>Хешрейт: {hr:.0f} MH/s\n"
            f"Прочность: {wear}%\n\n"
            f"✅ Успех: +{hr*OC_BONUS:.0f} MH/s → {hr*(1+OC_BONUS):.0f} MH/s\n"
            f"❌ Провал: уничтожение или -10% прочности</blockquote>\n\nТы уверен?")
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⚡ Разогнать!", callback_data=f"oc_do:{gid}")],
        [InlineKeyboardButton(text="❌ Отмена",     callback_data="overclock_menu")],
    ])
    try:
        await callback.message.edit_text(text, reply_markup=kb, parse_mode="HTML")
    except Exception:
...
@dp.callback_query(F.data.startswith("oc_do:"))
async def callback_oc_do(callback: CallbackQuery):
    await callback.answer()
    gid = int(callback.data.split(":")[1])
    c2 = db.conn.cursor()
    c2.execute("SELECT name,hash_rate,wear FROM user_gpus WHERE id=? AND user_id=?", (gid, callback.from_user.id))
    ...
    if not g:
        await callback.answer("GPU не найден", show_alert=True); return
    name, hr, wear = g[0], float(g[1] or 0), g[2] or 100
    if random.random() < OC_SUCCESS_CHANCE:
        new_hr = round(hr * (1 + OC_BONUS), 1)
        db.cursor.execute("UPDATE user_gpus SET hash_rate=? WHERE id=?", (new_hr, gid))
        db.conn.commit()
        text = (f"✅ <b>Разгон успешен!</b>\n\n"
                f"⚡ {name}\n"
                f"{hr:.0f} → {new_hr:.0f} MH/s (+{new_hr-hr:.0f})")
    else:
        if random.random() < OC_DESTROY_CHANCE:
            db.cursor.execute("DELETE FROM user_gpus WHERE id=?", (gid,))
            text = (f"💥 <b>Разгон провалился!</b>\n\n"
                    f"⚡ {name} сгорела!\nКарта уничтожена.")
        else:
            new_wear = max(0, wear - 10)
            db.cursor.execute("UPDATE user_gpus SET wear=? WHERE id=?", (new_wear, gid))
            text = (f"⚠️ <b>Разгон провалился!</b>\n\n"
                    f"⚡ {name}\nПрочность: {wear}% → {new_wear}%")
        db.conn.commit()
    db.recalculate_user_hashrate(callback.from_user.id)
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⚡ Ещё разгон", callback_data="overclock_menu")],
        [InlineKeyboardButton(text="« Назад",      callback_data="back_to_menu")],
    ])
    try:
        await callback.message.edit_text(text, reply_markup=kb, parse_mode="HTML")
...
#
#  Доступные игры:
#    🎰 Слоты     — /slots или кнопка
#    🎲 Кубы — /flip или кнопка
#    🎡 Рулетка  — /roulette или кнопка
#
# Лимиты: макс ставка 10 000 тон, минимум 10 тон

GAME_MIN_BET = 10
GAME_MAX_BET = 10_000

# ── Вспомогательная функция проверки ставки ───────────────────
def _check_bet(user, amount: int) -> str | None:
    """Возвращает текст ошибки или None если всё ок."""
    if amount < GAME_MIN_BET:
        return f"❌ Минимальная ставка {GAME_MIN_BET} тон"
    if amount > GAME_MAX_BET:
        return f"❌ Максимальная ставка {GAME_MAX_BET} тон"
    if int(user[4]) < amount:
        return f"❌ Недостаточно тон (есть {fmt(user[4])})"
    return None


def _deduct_bet(user_id: int, amount: int):
    db.cursor.execute("UPDATE users SET gems=gems-? WHERE user_id=?", (amount, user_id))
    db.conn.commit()


def _add_win(user_id: int, amount: int):
    db.cursor.execute("UPDATE users SET gems=gems+? WHERE user_id=?", (amount, user_id))
    db.conn.commit()


def _add_commission(amount: int):
    """5% с проигрышей идёт боту."""
    if amount > 0:
        db.cursor.execute("UPDATE bot_balance SET balance=balance+? WHERE id=1", (amount,))
        db.conn.commit()
...
async def show_game_menu(target):
    ...
    text = (
        "🎮 <b>Мини-игры</b>\n\n"
        "<blockquote>"
        "🎰 Слоты — выпадут три символа, угадай комбо\n"
        "🎲 Кубы — кто выкинет больше, выигрыш ×1.9\n"
        "🎡 Рулетка — красное/чёрное, число, зеро\n"
        "</blockquote>\n\n"
        "<i>Мин. ставка: 10 тон | Макс: 10 000 тон</i>"
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🎰 Слоты",      callback_data="game_slots")],
        [InlineKeyboardButton(text="🎲 Кубы", callback_data="game_flip")],
...
# ── Слоты: декодирование Telegram 🎰 dice value (1-64) ──────────
# Каждый барабан имеет 4 символа. Формула:
#   reel1 = (value-1) % 4 + 1
#   reel2 = (value-1)//4 % 4 + 1
#   reel3 = (value-1)//16 % 4 + 1
_SLOT_REEL = {1: "🍫", 2: "🍇", 3: "🍋", 4: "7️⃣"}

# Выплаты за три одинаковых
_SLOT_TRIPLE_PAYOUTS = {
    "7️⃣": 7.0,   # jackpot — val=64
    "🍋": 3.0,   # val=43
    "🍇": 2.0,   # val=22
    "🍫": 1.5,   # val=1
}
...
def _decode_slots(dice_value: int) -> tuple[list[str], float]:
    """Декодирует Telegram dice value (1-64) → (символы барабанов, множитель)."""
    v = dice_value - 1
    r1 = _SLOT_REEL[(v % 4) + 1]
    r2 = _SLOT_REEL[((v // 4) % 4) + 1]
    r3 = _SLOT_REEL[((v // 16) % 4) + 1]
    reels = [r1, r2, r3]
    # Три одинаковых
    if r1 == r2 == r3:
        return reels, _SLOT_TRIPLE_PAYOUTS.get(r1, 2.0)
    # Два одинаковых 7️⃣
...
def _slots_bet_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✏️ Введи ставку", callback_data="slots_custom")],
        [InlineKeyboardButton(text="« Игры",          callback_data="game_menu")],
    ])


@dp.callback_query(F.data == "game_slots")
async def callback_game_slots(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
...
async def callback_slots_custom(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    await state.set_state(GameStates.slots_waiting_bet)
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="« Игры", callback_data="game_menu")]
    ...
    await callback.message.edit_text(
        "🎰 Введи ставку (10–10 000 тон):", reply_markup=kb
    )


@dp.message(GameStates.slots_waiting_bet)
async def process_slots_custom_bet(message: Message, state: FSMContext):
    await state.clear()
    if not message.text or not message.text.isdigit():
        await message.answer("❌ Введи число")
        return
    await _do_slots(message, int(message.text))


@dp.callback_query(F.data.startswith("slots_bet:"))
async def callback_slots_bet(callback: CallbackQuery):
    await callback.answer()
    await _do_slots(callback, int(callback.data.split(":")[1]))


@dp.callback_query(F.data.startswith("slots_again:"))
async def callback_slots_again(callback: CallbackQuery):
    await callback.answer()
    await _do_slots(callback, int(callback.data.split(":")[1]))


async def _do_slots(target, amount: int):
    uid = target.from_user.id
    user = db.get_user(uid)
    err = _check_bet(user, amount)
    if err:
        if isinstance(target, Message):
    ...
    _deduct_bet(uid, amount)
    # Анимация 🎰 — результат берём из .dice.value
    chat_id = target.chat.id if isinstance(target, Message) else target.message.chat.id
    slot_msg = await bot.send_dice(chat_id, emoji="🎰")
    await asyncio.sleep(3)
    reels, mult = _decode_slots(slot_msg.dice.value)
    reel_str = " | ".join(reels)
    ...
    if mult > 0:
        ...
        profit = win - amount
        if mult >= 1.0:
            result_line = f"🏆 <b>Победа!</b> +{fmt(profit)} тон (×{mult})"
        else:
            result_line = f"🤏 Почти! Возврат {fmt(win)} тон (×{mult})"
    else:
        win = 0
        _add_commission(int(amount * 0.05))
        result_line = f"😔 Проигрыш. -{fmt(amount)} тон"

    text = (
        f"🎰 <b>Слоты</b>\n\n"
        f"<blockquote>[ {reel_str} ]</blockquote>\n\n"
        f"{result_line}"
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔄 Ещё раз",        callback_data=f"slots_again:{amount}")],
        [InlineKeyboardButton(text="🎰 Изменить ставку", callback_data="game_slots")],
        [InlineKeyboardButton(text="« Игры",         callback_data="game_menu")],
    ])
    if isinstance(target, Message):
        await target.answer(text, reply_markup=kb, parse_mode="HTML")
...
FLIP_MULT = 1.9  # Кубы: у кого больше — выигрыш ×1.9


@dp.callback_query(F.data == "game_flip")
async def callback_game_flip(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    await state.set_state(GameStates.flip_waiting_bet)
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="« Игры", callback_data="game_menu")]
    ])
    try:
        await callback.message.edit_text(
            f"🎲 <b>Кубы</b>\n\n"
            f"Ты и бот бросают по кубику — у кого больше, тот выигрывает ×{FLIP_MULT}\n\n"
            f"Введи ставку (10–10 000 тон):",
            reply_markup=kb, parse_mode="HTML"
        )
    except Exception:
        await callback.message.answer(
...
@dp.callback_query(F.data == "flip_custom")
async def callback_flip_custom(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    await state.set_state(GameStates.flip_waiting_bet)
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="« Игры", callback_data="game_menu")]
...
@dp.message(GameStates.flip_waiting_bet)
async def process_flip_custom_bet(message: Message, state: FSMContext):
    await state.clear()
    if not message.text or not message.text.isdigit():
        await message.answer("❌ Введи целое число"); return
    await _do_flip(message, int(message.text))


@dp.callback_query(F.data.startswith("flip_again:"))
async def callback_flip_again(callback: CallbackQuery):
    await callback.answer()
    amount = int(callback.data.split(":")[1])
    await _do_flip(callback, amount)


# Старые колбэки-заглушки для совместимости
@dp.callback_query(F.data.startswith("flip_bet:"))
async def callback_flip_bet(callback: CallbackQuery):
    await callback.answer()
    await _do_flip(callback, int(callback.data.split(":")[1]))


@dp.callback_query(F.data.startswith("flip_do:"))
async def callback_flip_do(callback: CallbackQuery):
    await callback.answer()
    await _do_flip(callback, int(callback.data.split(":")[1]))


async def _do_flip(target, amount: int):
    """Бросаем по кубику — у кого больше, тот выигрывает."""
    uid = target.from_user.id
    user = db.get_user(uid)
    err = _check_bet(user, amount)
    if err:
    ...
    # Анимация: два кубика (игрок и бот)
    try:
        d_player = await bot.send_dice(chat_id, emoji="🎲")
        d_bot    = await bot.send_dice(chat_id, emoji="🎲")
        await asyncio.sleep(3)
        player_roll = d_player.dice.value
        bot_roll    = d_bot.dice.value
    except Exception:
        player_roll = random.randint(1, 6)
        bot_roll    = random.randint(1, 6)

    # Ничья — перебрасываем программно
    while player_roll == bot_roll:
        player_roll = random.randint(1, 6)
        bot_roll    = random.randint(1, 6)

    if player_roll > bot_roll:
        win = int(amount * FLIP_MULT)
        _add_win(uid, win)
        profit = win - amount
        result_line = f"✅ <b>Победа!</b> +{fmt(profit)} тон"
    else:
        _add_commission(int(amount * 0.05))
        result_line = f"❌ Проигрыш. -{fmt(amount)} тон"
    ...
    text = (
        f"🎲 <b>Кубы</b>\n\n"
        f"<blockquote>"
        f"Твой кубик: {player_roll}\n"
        f"Кубик бота: {bot_roll}"
        f"</blockquote>\n\n"
        f"{result_line}"
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔄 Ещё раз",      callback_data=f"flip_again:{amount}")],
        [InlineKeyboardButton(text="🎲 Изменить ставку", callback_data="game_flip")],
        [InlineKeyboardButton(text="« Игры",            callback_data="game_menu")],
    ])
    if isinstance(target, Message):
        await target.answer(text, reply_markup=kb, parse_mode="HTML")
...
ROULETTE_BLACKS = set(range(1,37)) - ROULETTE_REDS


def _spin_roulette() -> int:
    return random.randint(0, 36)


def _roulette_result(number: int, bet_type: str) -> float:
    """Возвращает множитель выигрыша (0 = проигрыш)."""
    if bet_type == "red":
        return 1.9 if number in ROULETTE_REDS else 0
    if bet_type == "black":
        return 1.9 if number in ROULETTE_BLACKS else 0
    if bet_type == "zero":
        return 14.0 if number == 0 else 0
    # Конкретное число
...
def _roulette_bet_type_kb(amount: int):
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="🔴 Красное ×1.9",  callback_data=f"rou_do:{amount}:red"),
            InlineKeyboardButton(text="⚫ Чёрное ×1.9",   callback_data=f"rou_do:{amount}:black"),
        ],
        [
            InlineKeyboardButton(text="🟢 Зеро ×14",      callback_data=f"rou_do:{amount}:zero"),
            InlineKeyboardButton(text="🎯 Число ×35",     callback_data=f"rou_pick_num:{amount}"),
        ],
...
def _roulette_amounts_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✏️ Введи ставку", callback_data="rou_custom")],
        [InlineKeyboardButton(text="« Игры",          callback_data="game_menu")],
    ])


@dp.callback_query(F.data == "rou_custom")
async def callback_rou_custom(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    await state.set_state(GameStates.roulette_waiting_bet)
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="« Игры", callback_data="game_menu")]
...
async def process_rou_custom_bet(message: Message, state: FSMContext):
    await state.clear()
    if not message.text or not message.text.isdigit():
        await message.answer("❌ Введи число")
        return
    ...
    if err:
        ...
        return
    await message.answer(
        f"🎡 Ставка: <b>{fmt(amount)} тон</b>\n\nВыбери тип ставки:",
        reply_markup=_roulette_bet_type_kb(amount),
        parse_mode="HTML"
    )


@dp.callback_query(F.data.startswith("rou_bet:"))
async def callback_rou_bet(callback: CallbackQuery):
    await callback.answer()
    amount = int(callback.data.split(":")[1])
    try:
        await callback.message.edit_text(
...
@dp.callback_query(F.data.startswith("rou_pick_num:"))
async def callback_rou_pick_num(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    amount = int(callback.data.split(":")[1])
    await state.set_state(GameStates.roulette_waiting_number)
    await state.update_data(amount=amount)
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="« Назад", callback_data=f"rou_bet:{amount}")]
    ])
    await callback.message.edit_text(
        f"🎯 Введи число (0–36):",
        reply_markup=kb
...
@dp.message(GameStates.roulette_waiting_number)
async def process_rou_number(message: Message, state: FSMContext):
    data = await state.get_data()
    amount = data.get("amount", 0)
    await state.clear()
    if not message.text or not message.text.isdigit() or not (0 <= int(message.text) <= 36):
        await message.answer("❌ Введи число от 0 до 36")
        return
    await _do_roulette(message, amount, message.text)


@dp.callback_query(F.data.startswith("rou_do:"))
async def callback_rou_do(callback: CallbackQuery):
    await callback.answer()
    parts = callback.data.split(":")
    amount, bet_type = int(parts[1]), parts[2]
    await _do_roulette(callback, amount, bet_type)


async def _do_roulette(target, amount: int, bet_type: str):
    uid = target.from_user.id
    user = db.get_user(uid)
    err = _check_bet(user, amount)
    if err:
    ...
    # Анимация рулетки 🎡
    chat_id = target.chat.id if isinstance(target, Message) else target.message.chat.id
    rou_msg = await bot.send_dice(chat_id, emoji="🎡")
    await asyncio.sleep(4)
    number = _spin_roulette()
    color = _number_color(number)
    mult = _roulette_result(number, bet_type)
    ...
    bet_label = bet_labels.get(bet_type, f"🎯 Число {bet_type}")
    ...
    if bet_type in ("red", "black", "zero"):
        repeat_cb = f"rou_do:{amount}:{bet_type}"
        repeat_label = f"🔄 Ещё раз ({bet_label})"
    else:
        repeat_cb = f"rou_do:{amount}:{bet_type}"
        repeat_label = f"🔄 Ещё раз (число {bet_type})"

    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=repeat_label,           callback_data=repeat_cb)],
        [InlineKeyboardButton(text="🎡 Изменить ставку",   callback_data="game_roulette")],
        [InlineKeyboardButton(text="« Игры",               callback_data="game_menu")],
    ])
...
async def cmd_roulette(message: Message):
    ...
    await message.answer(
        "🎡 <b>Рулетка</b>\n\nВыбери ставку:",
        reply_markup=_roulette_amounts_kb(),
        parse_mode="HTML"
    )
...
@dp.message(Command("duel"))
async def cmd_duel_group(message: Message):
    """Вызов на дуэль в группе: /duel <ставка>"""
    if message.chat.type not in ("group", "supergroup"):
        await message.answer("⚔️ Дуэли доступны только в групповых чатах!")
        return
    args = message.text.split()
    if len(args) < 2 or not args[1].isdigit():
        await message.reply("⚔️ Использование: /duel <ставка>\nПример: <code>/duel 1000</code>", parse_mode="HTML")
        return
    amount = int(args[1])
    if amount < 1:
        await message.reply("❌ Минимальная ставка: 1 тон")
        return
    uid = message.from_user.id
    user = db.get_user(uid)
    if not user:
        await message.reply("❌ Ты не зарегистрирован! Напиши /start боту в личку.")
        return
    if int(user[4]) < amount:
        await message.reply(f"❌ Недостаточно тонов! У тебя: {fmt(int(user[4]))} тон")
        return
    challenger_name = escape_html(message.from_user.first_name)
    duel_id = f"{uid}_{message.message_id}"
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⚔️ Принять дуэль", callback_data=f"duel_accept:{duel_id}")]
    ])
    sent = await message.answer(
        f"⚔️ <b>Дуэль!</b>\n\n"
        f'<a href="tg://user?id={uid}">{challenger_name}</a> вызывает на дуэль!\n'
        f"💰 Ставка: <b>{fmt(amount)} тон</b>\n\n"
        f"Кто рискнёт принять? ⏳ 5 минут",
        parse_mode="HTML",
        reply_markup=kb
    )
...
async def callback_duel_accept(callback: CallbackQuery):
    ...
    if not duel:
        await callback.answer("❌ Дуэль устарела или уже завершена.", show_alert=True)
        return
    if datetime.now() > duel["expires"]:
        _active_duels.pop(duel_id, None)
        await callback.answer("❌ Время дуэли истекло.", show_alert=True)
        return
    acceptor_id = callback.from_user.id
    challenger_id = duel["challenger_id"]
    if acceptor_id == challenger_id:
        await callback.answer("❌ Нельзя принять свою же дуэль!", show_alert=True)
        return
    amount = duel["amount"]
    challenger_user = db.get_user(challenger_id)
    acceptor_user = db.get_user(acceptor_id)
    if not acceptor_user:
        await callback.answer("❌ Ты не зарегистрирован! Напиши /start боту в личку.", show_alert=True)
        return
    if not challenger_user or int(challenger_user[4]) < amount:
        _active_duels.pop(duel_id, None)
        await callback.answer("❌ У вызвавшего не хватает тонов!", show_alert=True)
        try:
            await callback.message.edit_text("⚔️ <b>Дуэль отменена</b> — у вызвавшего недостаточно тонов.", parse_mode="HTML")
        except Exception:
            pass
        return
    if int(acceptor_user[4]) < amount:
        await callback.answer(f"❌ Тебе не хватает тонов! Нужно: {fmt(amount)}", show_alert=True)
        return
    _active_duels.pop(duel_id, None)
    winner_id = random.choice([challenger_id, acceptor_id])
    loser_id = acceptor_id if winner_id == challenger_id else challenger_id
    db.cursor.execute("UPDATE users SET gems=gems+? WHERE user_id=?", (amount, winner_id))
    db.cursor.execute("UPDATE users SET gems=gems-? WHERE user_id=?", (amount, loser_id))
    db.conn.commit()
    challenger_name = duel["challenger_name"]
    acceptor_name = escape_html(callback.from_user.first_name)
    winner_name = challenger_name if winner_id == challenger_id else acceptor_name
    loser_name = acceptor_name if winner_id == challenger_id else challenger_name
    try:
        await callback.message.edit_text(
            f"⚔️ <b>Дуэль завершена!</b>\n\n"
            f"🥊 {challenger_name} vs {acceptor_name}\n"
            f"💰 Ставка: {fmt(amount)} тон\n\n"
            f"🏆 Победитель: <b>{winner_name}</b>\n"
            f"💸 +{fmt(amount)} тон → {winner_name}\n"
            f"💀 Проиграл: {loser_name}",
            parse_mode="HTML"
        )
    except Exception:
        pass
    result_text = "🏆 Ты победил!" if winner_id == acceptor_id else "💀 Ты проиграл!"
...
async def main():
    ...
    asyncio.create_task(auction_finalize_scheduler())
    asyncio.create_task(tournament_scheduler())
    asyncio.create_task(world_event_scheduler())
