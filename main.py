...
class Database:
    ...
    def mine(self, user_id):
        ...
        # Формируем ответ
        streak_line = f"\n🔥 Стрик: {streak+1} дн. (×{streak_mult:.2f})" if streak > 0 else ""
...
def get_help_keyboard():
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🛒 Магазин/Маркет", callback_data="help_shop"),
...
def get_donate_keyboard():
    ...
    rows = [
        ...
        # Ряд 2: Промокоды
        [
            InlineKeyboardButton(text="🎟 Промокод ника", callback_data="my_nick_promos"),
...
async def _buy_with_stars(callback: CallbackQuery, item_type: str, key: str):
    ...
    if stars >= price:
        ...
        db.conn.commit()
        if item_type == "priv":
            ok, msg = db.set_privilege(uid, key, price)
        else:
...
def get_top_keyboard():
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            ...
            InlineKeyboardButton(text="🔥 Стрики",   callback_data="top_streak"),
            InlineKeyboardButton(text="⚔️ Дуэли",   callback_data="top_duels"),
        ],
        [
            InlineKeyboardButton(text="« Назад", callback_data="back_to_menu"),
        ],
    ...
    if not results:
        ...
        for i, res in enumerate(results, 1):
            ...
            if len(res) >= 4:
                ...
                text += f"{icon} <a href=\"tg://user?id={user_id}\">{name_display}</a> — {value}\n"
...
async def world_event_scheduler():
    while True:
        ...
        try:
            ...
            c2 = db.conn.cursor()
            c2.execute("SELECT user_id FROM users WHERE last_mining > datetime('now','-1 day') AND banned=0 LIMIT 300")
            active_users = c2.fetchall()
            for (row_uid,) in active_users:
                if ev["effect"] == "instant_ton":
                    db.cursor.execute("UPDATE users SET gems=gems+? WHERE user_id=?", (int(ev["magnitude"]), row_uid))
                try:
                    await bot.send_message(row_uid, event_text, parse_mode="HTML")
                except Exception: pass
            if ev["effect"] == "instant_ton":
                db.conn.commit()
        except Exception as e:
